...
import nta.med.data.dao.medi.nur.Nur1012RepositoryCustom;

/**
 * @author dainguyen.
 */
public class Nur1012RepositoryImpl implements Nur1012RepositoryCustom {
	@PersistenceContext
	private EntityManager entityManager;
	
	@Override
	public String getNUR6011U01GetNurseInfoLifeSelfGrade(String hospCode, Double fkinp1001, String jukyongDate){
		StringBuilder sql = new StringBuilder();
		sql.append("   SELECT A.LIFE_SELF_GRADE                                                                           ");
		sql.append("     FROM NUR1012 A                                                                                   ");
		sql.append("    WHERE A.HOSP_CODE  = :f_hosp_code                                                                 ");
		sql.append("      AND A.FKINP1001  = :f_fkinp1001                                                                 ");
		sql.append("      AND A.JUKYONG_DATE = ( SELECT MAX(C.JUKYONG_DATE)                                               ");
		sql.append("                               FROM NUR1012 C                                                         ");
