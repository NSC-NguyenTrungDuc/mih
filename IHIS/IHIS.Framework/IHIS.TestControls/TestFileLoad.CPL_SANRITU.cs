...
namespace IHIS.TestControls
{
    ...
    internal class CplSanRituPtInfo
    {
        ...
        public Boolean Parse(String aServiceID, String aReceiveData)
        {
            ...
            try
            {
                ...
                //
                Logs.WriteLog(logFileTag, String.Format("Patient Information Parsing End=[{0}]", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:fff")));

                // 처리한 환자정보 다음 position = 검사결과정보 start position
                aReceiveData = Service.getWordByByte(aReceiveData, CONST_LINE_LEN);
                //
                #region 검사결과정보
                //Logs.WriteLog(logFileTag, "aCplSanRituPtInfo.insuCnt.Trim() [" + aCplSanRituPtInfo.insuCnt.Trim() + "]");
                ...
                try
                {
                    ...
                    for (int i = 0; i < cntResult; i++)
                    {
                        ...
                        //Logs.WriteLog(logFileTag, "Receive InsData " + "[" + insData + "]");
            ...
            {
                Logs.ErrWriteLog(logFileTag, String.Format("[CplSanRituPtInfo] Parse error[{0}][{1}]", xe.Message, xe.StackTrace));
                return false;
            }
        }
        ...
        public Boolean SetData(String aServiceID)
        {
            ...
            try
            {
                ...
                if (!Service.Connect())
                {
                    //Logs.ErrWriteLog(logFileTag, String.Format("[CplSanRituPtInfo] SetData : DB Connect Error[{0}]", DBService.ErrMsg));
                    Logs.ErrWriteLog(logFileTag, String.Format("[CplSanRituPtInfo] SetData : DB Connect Error[{0}]", Service.ErrMsg));
                    return false;
                }
                //Logs.WriteLog(logFileTag, "DB Connect");
                //
                ...
                if (retVal == null)
                {
                    //Logs.ErrWriteLog(logFileTag, String.Format("[CplSanRituPtInfo] SetData : Patient PK Creation Error :[{0}]", DBService.ErrFullMsg));
                    Logs.ErrWriteLog(logFileTag, String.Format("[CplSanRituPtInfo] SetData : Patient PK Creation Error :[{0}]", Service.ErrFullMsg));
                    return false;
                }
                //환자정보 PrimaryKey 취득
                ...
                if (bolval == false)
                {
                    ...
                    //Logs.ErrWriteLog(logFileTag, String.Format("[CplSanRituPtInfo] SetData Error:[{0}]", DBService.ErrFullMsg));
                    //DBService.sqlHelper.SQLErrmessagOutput(logFileTag, cmdText, bindVals);
                    Service.RollbackTransaction();
                    Logs.ErrWriteLog(logFileTag, String.Format("[CplSanRituPtInfo] SetData Error:[{0}]", Service.ErrFullMsg));
            ...
            catch (Exception xe)
            {
                //DBService.RollbackTransaction();
                Service.RollbackTransaction();
                Logs.ErrWriteLog(logFileTag, String.Format("[CplSanRituPtInfo] SetData Exception:[{0}][{1}]", xe.Message, xe.StackTrace));
                return false;
            }
            finally
            {
    ...
    internal class CplSanRituResult
    {
        ...
        public Boolean Parse(String aServiceID, String aReceiveData)
        {
            ...
            {
                Logs.ErrWriteLog(logFileTag, String.Format("[CplSanRituResult] Parse error[{0}][{1}]", xe.Message, xe.StackTrace));
                return false;
            }
        }
        ...
        public Boolean SetData(String aServiceID)
        {
            ...
            if (retVal == null)
            {
                //Logs.ErrWriteLog(logFileTag, String.Format("[CplSanRituResult] SetData : CplSanRituResult PK Creation Error :[{0}]", DBService.ErrFullMsg));
                Logs.ErrWriteLog(logFileTag, String.Format("[CplSanRituResult] SetData : CplSanRituResult PK Creation Error :[{0}]", Service.ErrFullMsg));
                return false;
            }
            //검사결과 PrimaryKey 취득
            this.PK = retVal.ToString();
            ...
            try
            {
                ...
                if (!Service.Connect())
                {
                    //Logs.ErrWriteLog(logFileTag, String.Format("[CplSanRituResult] SetData : DB Connect Error[{0}]", DBService.ErrMsg));
                    Logs.ErrWriteLog(logFileTag, String.Format("[CplSanRituResult] SetData : DB Connect Error[{0}]", Service.ErrMsg));
                    return false;
                }
                //Logs.WriteLog(logFileTag, "DB Connect");
                //
                ...
                {
                    ////DBService.RollbackTransaction();
                    //Logs.ErrWriteLog(logFileTag, String.Format("[CplSanRituResult] SetData Error:[{0}]", DBService.ErrFullMsg));
                    //DBService.sqlHelper.SQLErrmessagOutput(logFileTag, cmdText, bindVals);
                    //Service.RollbackTransaction();
                    Logs.ErrWriteLog(logFileTag, String.Format("[CplSanRituResult] SetData Error:[{0}]", Service.ErrFullMsg));
                    //Service.sqlHelper.SQLErrmessagOutput(logFileTag, cmdText, bindVals);
                    return false;
                }
                return true;
            ...
            {
                //DBService.RollbackTransaction();
                Logs.ErrWriteLog(logFileTag, String.Format("[CplSanRituResult] SetData Exception:[{0}][{1}]", xe.Message, xe.StackTrace));
                return false;
            }
        }
