...
-(void)initAccordion{
    ...
    scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, [self frame].size.width, [self frame].size.height - 60.0)];
...
- (void)setOriginalSize:(CGSize)size forIndex:(NSUInteger)index relayout:(BOOL)needLayout{
    if (index >= [views count]) return;
    [originalSizes replaceObjectAtIndex:index withObject:[NSValue valueWithCGSize:size]];
    if ([selectionIndexes containsIndex:index] && needLayout) {
        [self setNeedsLayout];
    }
    else{
...
#pragma mark - Move scroll view as content in view
// Position: end of cell need show
- (void)scrollToPosition:(float)position ofView:(id)view cellHeight:(float)cellHeight{
    if (self.isHorizontal) {
        // TODO
    } else {
        int height = 0;
        ...
        CGPoint offset = scrollView.contentOffset;
        float newOffsetY = height - scrollView.frame.size.height;
        if (offset.y < newOffsetY && offset.y > newOffsetY - cellHeight) {
            offset.y = newOffsetY;
            if (offset.y + scrollView.frame.size.height > height) {
                offset.y = scrollView.frame.size.height - height;
                if (offset.y < 0) {
                    offset.y = 0;
                }
            }
