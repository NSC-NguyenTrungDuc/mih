...
import nta.med.service.ihis.proto.PhysModelProto.PHY8002U00GrdPHY8002Info;
import nta.med.service.ihis.proto.PhysModelProto.PHY8002U00GrdPHY8003Info;
import nta.med.service.ihis.proto.PhysModelProto.PHY8002U00GrdPHY8004Info;
import nta.med.service.ihis.proto.PhysServiceProto;
import nta.med.service.ihis.proto.PhysServiceProto.PHY8002U00SaveLayoutRequest;
import nta.med.service.ihis.proto.PhysServiceProto.PHY8002U00SaveLayoutResponse;
...
@Service                                                                                                          
@Scope("prototype")                                                                                 
public class PHY8002U00SaveLayoutHandler 
	extends ScreenHandler<PhysServiceProto.PHY8002U00SaveLayoutRequest, PhysServiceProto.PHY8002U00SaveLayoutResponse> {                     
	private static final Log LOGGER = LogFactory.getLog(PHY8002U00SaveLayoutHandler.class);                                    
	@Resource                                                                                                       
	...
	@Override                                                                                                       
	public PHY8002U00SaveLayoutResponse handle(Vertx vertx, String clientId,
			String sessionId, long contextId,
			PHY8002U00SaveLayoutRequest request) throws Exception {                                                                  
      	   	PhysServiceProto.PHY8002U00SaveLayoutResponse.Builder response = PhysServiceProto.PHY8002U00SaveLayoutResponse.newBuilder();                      
	   	...
	   	String language = getLanguage(vertx, sessionId);
				...
				if(CollectionUtils.isEmpty(request.getPhy8003InfoList())){
					result8003 = true;
				}else{
					for(PHY8002U00GrdPHY8003Info  item : request.getPhy8003InfoList()){
				...
				// Caller_id = 3
				if(CollectionUtils.isEmpty(request.getPhy8004InfoList())){
					result8004 = true;
				}else{
					for(PHY8002U00GrdPHY8004Info item : request.getPhy8004InfoList()){
				...
				// Caller_id = 1
				if(CollectionUtils.isEmpty(request.getPhy8002InfoList())){
					result8002 = true;
				}else{
					for(PHY8002U00GrdPHY8002Info item : request.getPhy8002InfoList()){
					 ...
					 info = setPHY8002Info(info, request.getBunho(), request.getIoKubun(), hospCode, language);
						...
						String sinryouka = item.getSinryouka();
						String sindanisi = item.getSindanisi();
						String tantoui = item.getTantoui();
						if(item.getRowState().equalsIgnoreCase(DataRowState.UNCHANGED.getValue())){
							result8002 = true;
						}
						if(item.getRowState().equalsIgnoreCase(DataRowState.ADDED.getValue())){
						...
						imageSave = insertScan001(info, request.getUserId(), request.getBunho(), hospCode);
								if(!imageSave){
									break;
								}
					save = insertPhy8002(info, request.getUserId(), pkphy, sinryouka, sindanisi, tantoui, hospCode);
						}else if(item.getRowState().equalsIgnoreCase(DataRowState.MODIFIED.getValue())){
							if(!StringUtils.isEmpty(request.getCopyPkocskey())){
						imageSave = insertScan001(info, request.getUserId(), request.getBunho(), hospCode);
								if(!imageSave){
						...
						imageSave = insertScan001(info, request.getUserId(), request.getBunho(), hospCode);
								...
								save = updatePhy8002(info, request.getUserId(), pkphy);
							}
						} 
						if(save && imageSave){
							result8002 = true;
	...
	private PhysModelProto.PHY8002U00GrdPHY8002Info.Builder setPHY8002Info(PhysModelProto.PHY8002U00GrdPHY8002Info.Builder info, String bunho, String ioGubun, String hospCode, String language){
		...
		if(ioGubun.equalsIgnoreCase("I")){
			...
			if(!StringUtils.isEmpty(nyuuinnbi)){
				LOGGER.info("nyuuinnbi :"+nyuuinnbi);
	...
	// save 8003
	private boolean save8003(PHY8002U00GrdPHY8003Info  item, PhysServiceProto.PHY8002U00SaveLayoutRequest request, String hospCode){
		boolean save = false;
	...
	private boolean insertPhy8002(PhysModelProto.PHY8002U00GrdPHY8002Info.Builder item, String userId, String pkphy, String sinryouka,
			String sindanisi, String tantoui, String hospCode){
		...
		phy8002.setIoKubun(item.getIoKubun());
		...
		phy8002.setCrTime(new Date());
		phy8002.setReha1(item.getReha1());
		phy8002.setReha2(item.getReha2());
		phy8002.setReha3(item.getReha3());
		phy8002.setReha4(item.getReha4());
		phy8002.setReha5(item.getReha5());
		phy8002Repository.save(phy8002);
		return true;
	}
	
	private boolean insertScan001(PhysModelProto.PHY8002U00GrdPHY8002Info.Builder item, String userId, 
	...
	private boolean updatePhy8002(PhysModelProto.PHY8002U00GrdPHY8002Info.Builder item, String userId, String pkphy){
		if(phy8002Repository.updatePhy8002U00(
				...
				DateUtil.toDate(item.getCrTime(), DateUtil.PATTERN_SLASH_YYYYMMDD_HH_COLON_MM_SS),
				item.getReha1(),
				item.getReha2(),
				item.getReha3(),
				item.getReha4(),
	...
	private boolean insertPhy8003(PHY8002U00GrdPHY8003Info item, String userId, String pkChk, String hospCode){
	...
	private boolean update8003(PHY8002U00GrdPHY8003Info item, String userId){
	...
	private boolean insert8004(PHY8002U00GrdPHY8004Info item , String userId, String pkChk, String hospCode){
