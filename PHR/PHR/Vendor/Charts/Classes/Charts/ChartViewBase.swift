...
public class ChartViewBase: NSUIView, ChartDataProvider, ChartAnimatorDelegate
{
    ...
    public var infoFont: NSUIFont! = NSUIFont(name: "Aileron-Regular", size: 16.0)
    ...
    public var isShowNoDataText = false
    ...
    public var _isShowNoDataText: Bool
    {
        get
        {
            return isShowNoDataText
        }
        set
        {
            isShowNoDataText = newValue
            notifyDataSetChanged()
        }
    }
        ...
        for i in 0 ..< _indicesToHighlight.count
        {
            ...
            if xIndex <= Int(deltaX) && xIndex <= Int(CGFloat(deltaX) * _animator.phaseX)
            {
                ...
                if (pos.y - markerSize.height <= 0.0)
                {
                    ...
                    if (highlight.xIndex == 0) {
                        marker!.draw(context: context, point: CGPoint(x: pos.x + 5, y: pos.y + y))
                    } else if (highlight.xIndex == data!.getDataSetByIndex(highlight.dataSetIndex).entryCount - 1){
                        marker!.draw(context: context, point: CGPoint(x: pos.x - 5, y: pos.y + y))
                    } else {
                ...
                {
                    if (highlight.xIndex == 0) {
                        marker!.draw(context: context, point: CGPoint(x: pos.x + 5, y: pos.y))
                    } else if (highlight.xIndex == data!.getDataSetByIndex(highlight.dataSetIndex).entryCount - 1){
                        marker!.draw(context: context, point: CGPoint(x: pos.x - 5, y: pos.y))
                    } else {
