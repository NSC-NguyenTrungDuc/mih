...
#import "BodyMeasurementDetailChart.h"

@implementation BodyMeasurementDetailChart {
    float maxLineYAxis;
    float average;
    NSString *chartUnit;
    NSString *chartTitle;
...
- (void)doCustomize{
    ...
    self.arrayChartBackgroundColor = @[[UIColor colorWithRed:175./255. green:159./255. blue:89./255. alpha:0],
                                       [UIColor colorWithRed:140./255. green:191./255. blue:153./255. alpha:0],
                                       [UIColor colorWithRed:170./255. green:143./255. blue:208./255. alpha:0],
                                       [UIColor colorWithRed:178/255. green:149/255. blue:49/255. alpha:0]
                                       ];
    if (self.chartContentType == ChartContentTypeHeight) {
        chartUnit = kLocalizedString(kUnitCm);
        chartTitle = kLocalizedString(kHeightUppercase);
    ...
    if (self.chartContentType == ChartContentTypeWeight) {
        ...
        chartTitle = kLocalizedString(kWeightUppercase);
    ...
    if (self.chartContentType == ChartContentTypeBMI) {
        ...
        chartTitle = kLocalizedString(kBodyMassIndex);
...
- (void)calculateMinMax {
    ...
    // WEIGHT
    else if (self.chartContentType == ChartContentTypeWeight) {
        if (self.arrayAllData.count > 0) {
            for (BodyMeasurementModel *obj in self.arrayAllData){
    ...
    // BODY FAT
    else if (self.chartContentType == ChartContentTypeBodyFat) {
        if (self.arrayAllData.count > 0) {
            for (BodyMeasurementModel *obj in self.arrayAllData){
    ...
    // BMI
    else if (self.chartContentType == ChartContentTypeBMI) {
        if (self.arrayAllData.count > 0) {
            for (BodyMeasurementModel *obj in self.arrayAllData){
