...
- (void)setupGroup:(voidBlock)endblock withSetupAsset:(BOOL)doSetupAsset
{
    ...
    ALAssetsLibraryAccessFailureBlock failureBlock = ^(NSError *error) {
        ...
        [strongSelf setTitle:kLocalizedString(kNotAllowed)];
...
//    locked.contentMode      = UIViewContentModeCenter;
    ...
    UILabel *title          = [[UILabel alloc] initWithFrame:rect];
    UILabel *message        = [[UILabel alloc] initWithFrame:rect];
    UILabel *guide1         = [[UILabel alloc] initWithFrame:rect];
    UILabel *guide2         = [[UILabel alloc] initWithFrame:rect];
    UIButton *btnSetting    = [[UIButton alloc] initWithFrame:CGRectMake(rect.origin.x, rect.origin.y, 200, 40)];
    
    title.text              = NSLocalizedStringFromTable(@"User need accept allow photo", @"UzysAssetsPickerController",nil);
    title.font              = [FontUtils aileronBoldWithSize:17.];
    title.textColor         = [UIColor colorWithRed:129.0/255.0 green:136.0/255.0 blue:148.0/255.0 alpha:1];
    title.textAlignment     = NSTextAlignmentCenter;
    title.numberOfLines     = 5;
    
    message.text            = NSLocalizedStringFromTable(@"The photo share of from the library to allow the photos save to the camera roll.", @"UzysAssetsPickerController",nil);
    message.font            = [FontUtils aileronRegularWithSize:14.];
    message.textColor       = [UIColor colorWithRed:129.0/255.0 green:136.0/255.0 blue:148.0/255.0 alpha:1];
    message.textAlignment   = NSTextAlignmentCenter;
    message.numberOfLines   = 0;
    message.lineBreakMode   = NSLineBreakByWordWrapping;
    
    guide1.text            = NSLocalizedStringFromTable(@"1. Go to Setting", @"UzysAssetsPickerController",nil);
    guide1.font            = [FontUtils aileronRegularWithSize:14.];
    guide1.textColor       = [UIColor colorWithRed:129.0/255.0 green:136.0/255.0 blue:148.0/255.0 alpha:1];
    guide1.textAlignment   = NSTextAlignmentCenter;
    guide1.numberOfLines   = 5;
    
    guide2.text            = NSLocalizedStringFromTable(@"2. On Photo Setting", @"UzysAssetsPickerController",nil);
    guide2.font            = [FontUtils aileronRegularWithSize:14.];
    guide2.textColor       = [UIColor colorWithRed:129.0/255.0 green:136.0/255.0 blue:148.0/255.0 alpha:1];
    guide2.textAlignment   = NSTextAlignmentCenter;
    guide2.numberOfLines   = 5;
    
    [btnSetting setTitle:NSLocalizedStringFromTable(@"Allow access", @"UzysAssetsPickerController",nil) forState:UIControlStateNormal];
    [btnSetting setBackgroundColor:[UIColor clearColor]];
    [btnSetting setTitleColor:PHR_COLOR_CYAN forState:UIControlStateNormal];
    [btnSetting setTitleColor:PHR_COLOR_GRAY forState:UIControlStateHighlighted];
    [btnSetting.titleLabel setFont:[FontUtils aileronSemiBoldWithSize:16.]];
    ...
    title.center           = CGPointMake(lockedView.center.x, lockedView.center.y - 70);
    message.center         = title.center;
    guide1.center          = title.center;
    guide2.center          = title.center;
    btnSetting.center      = title.center;
    ...
    rect                    = guide1.frame;
    rect.origin.y           = message.frame.origin.y + message.frame.size.height + 10;
    guide1.frame           = rect;
    
    rect                    = guide2.frame;
    rect.origin.y           = guide1.frame.origin.y + guide1.frame.size.height + 5;
    guide2.frame           = rect;
    
    rect                    = btnSetting.frame;
    rect.origin.y           = guide2.frame.origin.y + guide2.frame.size.height + 25;
    btnSetting.frame        = rect;
    
    
    [lockedView addSubview:title];
    [lockedView addSubview:message];
    [lockedView addSubview:guide1];
    [lockedView addSubview:guide2];
    [lockedView addSubview:btnSetting];
    [self.collectionView addSubview:lockedView];
