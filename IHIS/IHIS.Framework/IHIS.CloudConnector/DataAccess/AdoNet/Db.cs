...
namespace IHIS.CloudConnector.DataAccess.AdoNet
{
    ...
    public static class Db
    {
        ...
        private static readonly string dataProviderMisa = ConfigurationManager.AppSettings.Get("DataProviderMisa");
        private static readonly DbProviderFactory factoryMisa = DbProviderFactories.GetFactory(dataProviderMisa);

        private static readonly string connectionStringName = ConfigurationManager.AppSettings.Get("ConnectionStringName");
        private static readonly string connectionString = "Data Source=" + Environment.CurrentDirectory + ConfigurationManager.ConnectionStrings[connectionStringName].ConnectionString;
        private static readonly string connectionStringMisa = String.Format("Server={0}\\{1};Database={2};User Id={3};Password={4};", UserInfo.MisaIp, UserInfo.MisaInstanceName, UserInfo.MisaDbInsurName, UserInfo.MisaUser, UserInfo.MisaPwd);
        ...
        public static DbConnection CreateConnection(IDao dao)
        {
            ...
            if (dao == null)
            {
                ...
                switch (dao.Source)
                {
                    ...
                    case DataSource.MISA:
                        connection = factoryMisa.CreateConnection();
                        connection.ConnectionString = connectionStringMisa;
                        break;
                }
        ...
        public static DbCommand CreateCommand(IDao dao)
        {
            if (dao == null)
            {
            ...
            else
            {
                switch (dao.Source)
                {
                    case DataSource.CACHE:
                        return factory.CreateCommand();
                    case DataSource.MISA:
                        return factoryMisa.CreateCommand();
                    default:
                        return factory.CreateCommand();
                }
        ...
        {
            return CreateParameter(name, type, value, null);
        ...
        public static DbParameter CreateParameter(string name, DbType type, object value, IDao dao)
        {
            ...
            else
            {
                switch (dao.Source)
                {
                    case DataSource.CACHE:
                        ...
                        break;
                    case DataSource.MISA:
                        parameter = factoryMisa.CreateParameter();
                        break;
                }
            }
            parameter.ParameterName = name;
        ...
        public static DbDataAdapter CreateDataAdapter(IDao dao)
        {
            if (dao == null)
            {
            ...
            else
            {
                switch (dao.Source)
                {
                    case DataSource.CACHE:
                        ...
                        //break;
                    case DataSource.MISA:
                        return factoryMisa.CreateDataAdapter();
                        //break;
                    default:
                        return factory.CreateDataAdapter();
                }
        ...
        /// <param name="sql">Sql statement.</param>
        /// <returns>Number of rows affected.</returns>
        public static int Update(string sql, IDao dao)
        {
            if (dao == null)
            {
                using (DbConnection connection = factory.CreateConnection())
            ...
            else
            {
                switch (dao.Source)
                {
                    ...
                    case DataSource.MISA:
                        using (DbConnection connection = factoryMisa.CreateConnection())
                        {
                            connection.ConnectionString = connectionString;

                            using (DbCommand command = factoryMisa.CreateCommand())
                            {
                                command.Connection = connection;
                                command.CommandText = sql;
