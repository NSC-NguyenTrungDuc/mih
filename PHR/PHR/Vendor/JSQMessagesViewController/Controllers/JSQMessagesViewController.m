...
- (void)jsq_configureMessagesViewController
{
    ...
    self.inputToolbar.contentView.textView.placeHolderTextColor = PHR_COLOR_GRAY;
...
- (void)finishReceivingMessageAnimated:(BOOL)animated {
    ...
    UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, kLocalizedString(@"new_message_received_accessibility_announcement"));
}
...
- (UICollectionViewCell *)collectionView:(JSQMessagesCollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!isMediaMessage) {
        ...
        cell.cellBottomLabel.text = [messageItem senderDisplayName];
    ...
    if (isOutgoingMessage) {
      //TungNT: kJSQMessagesCollectionViewAvatarSizeDefault + 25
        cell.cellBottomLabel.textInsets = UIEdgeInsetsMake(0, 0, 0,kJSQMessagesCollectionViewAvatarSizeDefault + 25);
    }
    else {
      //TungNT: kJSQMessagesCollectionViewAvatarSizeDefault+ 25
        cell.cellBottomLabel.textInsets = UIEdgeInsetsMake(0, 25, 0,kJSQMessagesCollectionViewAvatarSizeDefault + 0);
    }
               ...
               message:(id<JSQMessageData>)messageItem
{
    ...
    if (!isMediaMessage) {
        cell.accessibilityLabel = [NSString stringWithFormat:kLocalizedString(@"text_message_accessibility_label"),
                                   [messageItem senderDisplayName],
                                   [messageItem text]];
    }
    else {
        cell.accessibilityLabel = [NSString stringWithFormat:kLocalizedString(@"media_message_accessibility_label"),
                                   [messageItem senderDisplayName]];
    }
}
...
- (NSMutableAttributedString *) setColoredLabelWithNormalTitle: (NSString *)title andColoredTitle:(NSString *)coloredTitle{
    NSMutableAttributedString *text = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@%@",title, coloredTitle]];
    [text addAttribute: NSForegroundColorAttributeName value: [UIColor redColor] range: NSMakeRange(0, title.length)];
    [text addAttribute: NSForegroundColorAttributeName value: [UIColor purpleColor] range: NSMakeRange(title.length, coloredTitle.length)];
    return text;
}
