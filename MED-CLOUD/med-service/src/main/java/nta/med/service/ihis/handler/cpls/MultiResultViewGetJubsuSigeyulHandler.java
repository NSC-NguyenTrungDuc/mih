...
import nta.med.service.ihis.proto.CplsServiceProto.MultiResultViewGetJubsuSigeyulRequest;
...
@Scope("prototype")
@Transactional
public class MultiResultViewGetJubsuSigeyulHandler extends ScreenHandler<CplsServiceProto.MultiResultViewGetJubsuSigeyulRequest, CplsServiceProto.MultiResultViewLaySigeyulResponse> {                     
	...
	public MultiResultViewLaySigeyulResponse handle(Vertx vertx,
			...
			MultiResultViewGetJubsuSigeyulRequest request) throws Exception  {                                                                   
		...
		if(!CollectionUtils.isEmpty(grdSigeyul)){
			for(int i = 0;i < grdSigeyul.size(); i++){
				...
				List<MultiResultViewSearchDataSigeyulInfo> listSearchData = cpl2010Repository.getMultiResultViewSearchSigeyulInfo1(hospCode
