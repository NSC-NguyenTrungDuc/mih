...
<c:url var="backUrl" value="sms-template-list"/>
<c:url var="confirmUrl" value="sms-template-detail"/>
<form:form method="post" class="form-horizontal" action="${confirmUrl}" modelAttribute="mailTemplate" >
	...
	<div class="form-group">
		...
		<div class="col-xs-12 col-sm-10">
			<form:textarea path="content" class="form-control" rows="15" value="${mailTemplate.content}"/>
		</div>
	</div>
			...
			<div class="modal-content">
				<div class="modal-body">
					...
					<p>
						<c:choose>
							<c:when test="${not empty mailTemplate.mailTemplateId}">
								<spring:message code="be0311.modal.sms_template.edit.title" />	
							</c:when>
							<c:otherwise>
								<spring:message code="be0311.modal.sms_template.create.title" />	
							</c:otherwise>
						</c:choose>
					</p>
				</div>
	    		 ...
	    		 <div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">
					 ...
					 <h4 class="modal-title" align="left"> 
						<label class="modal-title" align="center"><spring:message code="be0301.modal.desc.parameter" /></label>
					</h4>
	    		 </div>
	    		<div class="modal-body" style="height: 600px; overflow: scroll">
		      		<%@ include file="../view/SmsParameterDescription.jsp" %>
				</div>
	    	</div>
  		</div>
	</div>
