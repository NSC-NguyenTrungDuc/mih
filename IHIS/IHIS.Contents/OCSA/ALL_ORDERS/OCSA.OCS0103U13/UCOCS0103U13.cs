...
namespace IHIS.OCSA
{
    public partial class UCOCS0103U13 : XScreen
    {
        #region Constructor
        /// <summary>
        /// Constructor
        ...
        public UCOCS0103U13(bool initialize)
        {
            // MessageBox.Show("Constructor start");
        ...
        private int OrderNormalWidth = 662;
        ...
        private void InitScreen()
        {
            ...
            this.grdSpecimen.AutoSizeColumn(1, 0); // 項目コード
        ...
        private void SetSizeColumByLanguage()
        {
            ...
            {
                grdOrder.AutoSizeColumn(3, 36);
                grdOrder.AutoSizeColumn(4, 70);
                grdOrder.AutoSizeColumn(9, 37);
                grdOrder.AutoSizeColumn(10, 36);
                grdOrder.AutoSizeColumn(11, 39);
                grdOrder.AutoSizeColumn(13, 33);
                grdOrder.AutoSizeColumn(14, 39);
                grdOrder.AutoSizeColumn(15, 31);
                grdOrder.AutoSizeColumn(16, 29);
                grdOrder.AutoSizeColumn(17, 29);
                grdOrder.AutoSizeColumn(19, 55);
                grdOrder.AutoSizeColumn(23, 30);
                grdOrder.AutoSizeColumn(25, 30);
                grdOrder.AutoSizeColumn(26, 30);             
            }
            else
            {
                grdOrder.AutoSizeColumn(3, 91);
                grdOrder.AutoSizeColumn(4, 127);
                grdOrder.AutoSizeColumn(9, 60);
                grdOrder.AutoSizeColumn(10, 60);
                grdOrder.AutoSizeColumn(11, 50);
                grdOrder.AutoSizeColumn(13, 50);
                grdOrder.AutoSizeColumn(14, 60);
                grdOrder.AutoSizeColumn(15, 60);
                grdOrder.AutoSizeColumn(16, 55);
                grdOrder.AutoSizeColumn(17, 41);
                grdOrder.AutoSizeColumn(19, 74);
                grdOrder.AutoSizeColumn(23, 30);
                grdOrder.AutoSizeColumn(25, 60);
                grdOrder.AutoSizeColumn(26, 60);  
            }
        }
        ...
        private void SetGroupControl(Hashtable aGroupInfo)
        {
            ...
            cbxOutCommon.CheckedChanged -= cbxOutCommon_CheckedChanged;
        ...
        private void SetBookingOtherClinic(Hashtable aGroupInfo)
        {
            for (int k = 0; k < this.grdOrder.RowCount; k++)
            {
                if (this.grdOrder.GetItemString(k, "group_ser") == aGroupInfo["group_ser"].ToString())
                {
                    ...
                    {
                        this.cbxOutCommon.SetDataValue("Y");
                        return;
                    }
                }
            }

            this.cbxOutCommon.SetDataValue("N");
        }
        ...
        private void ApplyCopyOrderInfo(MultiLayout aSourceLayout, OCS.HangmogInfo.ExecutiveMode aMode)
        {
            ...
            {
                if (this.mHangmogInfo.LoadHangmogInfo(aMode, aSourceLayout, true) == false)
                {
                    return;
                }
            }
        ...
        private void OverrideLookAndFeel()
        {
            ...
            grdSpecimen.Size = new Size(254, 485);
            tvwSpecimenBunryu.Size = new Size(196, 485);
            pnlSpecimen.Size = new Size(450, 485);
            grdSearchOrder.Size = new Size(450, 485);
            pnlSearch.Size = new Size(450, 485);
        ...
        private void LoadCommonOrderInfo(string aSearchWord, string aGijunDate, string aInputTab, string aWonnaeDrgYn)
        {
            this.grdSearchOrder.Reset();
            this.grdSearchOrder.SetBindVarValue("f_search_word", aSearchWord);
            this.grdSearchOrder.SetBindVarValue("f_gijun_date", aGijunDate);
            ...
            this.grdSearchOrder.SetBindVarValue("f_wonnae_drg_yn", aWonnaeDrgYn);
            this.grdSearchOrder.SetBindVarValue("f_search_condition", this.cboSearchCondition.SelectedValue.ToString());
            this.grdSearchOrder.ExecuteQuery = ExecuteLoadCommonOrderInfo;
            this.grdSearchOrder.QueryLayout(false);
        }
        ...
        {
            // 분류 데이터 로드 
            this.LoadSpecimenBunryu();

            this.tvwSpecimenBunryu.AfterSelect -= new TreeViewEventHandler(tvwDrgBunryu_AfterSelect);

            this.tvwSpecimenBunryu.Nodes.Clear();
            ...
            foreach (DataRow dr in this.laySpecimenTree.LayoutTable.Rows)
            {
                if (currentParent != dr["code"].ToString())
                {
                    //if (currentParent != "")
                    //{
                    //    this.tvwSpecimenBunryu.Nodes.Add(parentNode);
                    //}

                    parentNode = new TreeNode(dr["code_name"].ToString(), 5, 6);
                    parentNodeInfo = new Hashtable();
                    parentNodeInfo.Add("cpl_code_yn", dr["cpl_code_yn"].ToString());
                    parentNodeInfo.Add("code", dr["code"].ToString());
                    //parentNodeInfo.Add("code1", dr["code1"].ToString());
                    ...
                    childNodeInfo.Add("cpl_code_yn", dr["cpl_code_yn"].ToString());
                    ...
                    //Treeで院内検査外注検査を色で区別するためにNodeのTextの中に
                    //”院内”の文字が入っているのかをチェックし、入っていたらピンク色を
                    //”外”の文字が入っていたら青い色を塗る
                    if (parentNode.Text == "セット検査" && childNode.Text.IndexOf("院内") > -1)
                        childNode.BackColor = Color.LightPink;
                    else if (parentNode.Text == "セット検査" && childNode.Text.IndexOf("外") > -1)
                        childNode.BackColor = Color.LightBlue;
                    //
                    tvwSpecimenBunryu.Nodes.Add(parentNode);
            ...
            //insert by jc 初期値を院内検査に設定。　START
            string wonnaeDrgYn = this.cboQueryCon.GetDataValue();
            if (this.txtSearch.GetDataValue() == "")
            {
                //((Hashtable)tvwSpecimenBunryu.Nodes[0].Tag)["code"]
                //if (tvwSpecimenBunryu.Nodes[0].Text == "セット検査")
                //    this.tvwSpecimenBunryu.SelectedNode = tvwSpecimenBunryu.Nodes[0].Nodes[0];
                //else
                //    this.tvwSpecimenBunryu.SelectedNode = tvwSpecimenBunryu.Nodes[0].Nodes[1];
            }
            else
            {
                //this.tvwSpecimenBunryu.SelectedNode = tvwSpecimenBunryu.Nodes[0].Nodes[0];
            ...
            //this.LoadSpecimenOrder("1", this.tvwSpecimenBunryu.SelectedNode.Tag.ToString(), wonnaeDrgYn, "");
            //this.LoadSpecimenOrder("1", (Hashtable)this.tvwSpecimenBunryu.SelectedNode.Tag, wonnaeDrgYn, "");

            //if (this.tvwSpecimenBunryu.SelectedNode.Tag.ToString() == "System.Collections.Hashtable")
            //this.LoadSpecimenOrder("1", (Hashtable)this.tvwSpecimenBunryu.SelectedNode.Tag, wonnaeDrgYn, "");
            //else
            //    this.LoadSpecimenOrder("1", this.tvwSpecimenBunryu.SelectedNode.Tag.ToString(), wonnaeDrgYn, "");

            PostCallHelper.PostCall(new PostMethod(this.specimenChangedCheck));
            //insert by jc 初期値を院内検査に設定。　END
        ...
        private void specimenChangedCheck()
        {
            ...
            //if (this.txtSearch.GetDataValue() != "")
            //{
            //    this.tvwSpecimenBunryu.SelectedNode = tvwSpecimenBunryu.Nodes[0];

            //    this.LoadSpecimenOrder("2", "%", this.cboQueryCon.GetDataValue(), this.txtSearch.GetDataValue());
            //}

            for (int k = 0; k < this.grdSpecimen.RowCount; k++)
            {
                grdSpecimen.SetItemValue(k, "select_yn", "N");
                image_num = this.ImageList.Images[1];
                grdSpecimen[k, "select_yn"].Image = image_num;
            }
            ...
            for (int i = 0; i < this.grdSpecimen.RowCount; i++)
            {
                ...
                {
                    if ((this.grdSpecimen.GetItemString(i, "hangmog_code") == this.grdOrder.GetItemString(j, "hangmog_code"))
                        && this.grdOrder.GetItemString(j, "acting_date") == ""
                        && group_number == this.grdOrder.GetItemString(j, "group_ser"))
                    {
                        grdSpecimen.SetItemValue(i, "select_yn", "Y");
                        image_num = this.ImageList.Images[0];
                        grdSpecimen[i, "select_yn"].Image = image_num;
                    }
                }
            }
        ...
        private void LoadSpecimenBunryu()
        {
            this.laySpecimenTree.SetBindVarValue("f_hosp_code", EnvironInfo.HospCode);
            this.laySpecimenTree.QueryLayout(true);
        }

        private void LoadSpecimenOrder(string aMode, string aCode1, string aWonnaeDrgYn, string aSearchWord)
        {
            string searchword = "";
            ...
            if (aSearchWord != "%" && aSearchWord != "")
            {
                ...
                if (this.tvwSpecimenBunryu.SelectedNode.Level == 1 && (this.tvwSpecimenBunryu.SelectedNode.Parent.Text == "セット検査" || this.tvwSpecimenBunryu.SelectedNode.Parent.Text == "セット検査(個人)"))
                    this.grdSpecimen.SetBindVarValue("f_cpl_code_yn", "Y");
            }
            //Fix bug MED-9113
            //else if (aSearchWord == "")
            //{
            ...
            this.grdSpecimen.SetBindVarValue("f_mode", aMode);
            this.grdSpecimen.SetBindVarValue("f_slip_code", aCode1);
            this.grdSpecimen.SetBindVarValue("f_specimen_code", "%");
            this.grdSpecimen.SetBindVarValue("f_hosp_code", mHospCode);
            this.grdSpecimen.SetBindVarValue("f_input_tab", INPUTTAB);
            this.grdSpecimen.SetBindVarValue("f_search_word", searchword);
            this.grdSpecimen.SetBindVarValue("f_wonnae_drg_yn", aWonnaeDrgYn);
            this.grdSpecimen.SetBindVarValue("f_user", UserInfo.UserID);

            if (this.mOrderMode == OrderVariables.OrderMode.InpOrder || this.mOrderMode == OrderVariables.OrderMode.OutOrder)
                this.grdSpecimen.SetBindVarValue("f_order_date", this.mOrderDate);
            else
                this.grdSpecimen.SetBindVarValue("f_order_date", EnvironInfo.GetSysDate().ToString("yyyy/MM/dd").Replace("-", "/"));
        ...
        private void LoadSpecimenOrder(string aMode, Hashtable aNodeInfo, string aWonnaeDrgYn, string aSearchWord)
        {
            ...
            this.grdSpecimen.SetBindVarValue("f_mode", aMode);
            this.grdSpecimen.SetBindVarValue("f_slip_code", aNodeInfo["code1"].ToString());
            this.grdSpecimen.SetBindVarValue("f_cpl_code_yn", aNodeInfo["cpl_code_yn"].ToString());
            this.grdSpecimen.SetBindVarValue("f_specimen_code", "%");
            this.grdSpecimen.SetBindVarValue("f_hosp_code", mHospCode);
            this.grdSpecimen.SetBindVarValue("f_input_tab", INPUTTAB);
            this.grdSpecimen.SetBindVarValue("f_search_word", searchword);
        ...
        private void grdChangedSetSpecimenGid()
        {
            Image image_num;
            string group_number = "";
            int currentRow = this.grdSpecimen.CurrentRowNumber;
            ...
            string hangmog_code = grdSpecimen.GetItemString(currentRow, "hangmog_code");

            for (int i = 0; i < grdOrder.RowCount; i++)
            {
                if (grdOrder.GetItemString(i, "hangmog_code") == hangmog_code
                    && grdOrder.GetItemString(i, "acting_date") == ""
                    && group_number == grdOrder.GetItemString(i, "group_ser"))
                {
                    grdSpecimen.SetItemValue(currentRow, "select_yn", "Y");
                    image_num = this.ImageList.Images[0];
                    grdSpecimen[currentRow, "select_yn"].Image = image_num;
                }
            }
            this.grdSpecimen.Refresh();
        }
        ...
        public void HandleBtnListClick(FunctionType fType)
        {
            switch (fType)
            {
                ...
                case FunctionType.Update:
                    ...
                    //MED-10204
                    if (!ValidateCommonOrder())
                        return;

                    cbxOutCommon_CheckedChanged(new object(), new EventArgs());
                ...
                case FunctionType.Close:
                    ...
                    //https://sofiamedix.atlassian.net/browse/MED-11981
                    this.mContainer.Close();
                    break;
            }
        }

        private bool ValidateCommonOrder()
        {
            try
            {
                    ...
                    if (HasCommonOrderInGroup(tabInfo["group_ser"].ToString()))
                    {
                        for (int k = 0; k < this.grdOrder.RowCount; k++)
                        {
                            if (this.grdOrder.GetItemString(k, "group_ser") == tabInfo["group_ser"].ToString())
                            {
                                ...
                                {
                                    XMessageBox.Show(Resources.MSG7, Resources.MSG1_CAP, MessageBoxIcon.Error);
                                    return false;
                                }
                            }
                        }
            ...
            {
                Service.WriteLog("MED-10204 error: " + ex.Message);
                Service.WriteLog("MED-10204 stack trace: " + ex.StackTrace);
                return false;
            }
        }
        ...
        private void tvwDrgBunryu_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            {
                string wonnaeDrgYn = this.cboQueryCon.GetDataValue();
                this.LoadSpecimenOrder("1", ((Hashtable)e.Node.Tag), wonnaeDrgYn, "");
            }
            //insert by jc START
            PostCallHelper.PostCall(new PostMethod(this.specimenChangedCheck));
            //insert by jc END
        ...
        private void btnExpandSearch_Click(object sender, EventArgs e)
        {
            if (this.mIsSearchExtended == false)
            {
                ...
                this.grdSpecimen.AutoSizeColumn(2, this.SpecimenHangmogNameMaxWidth);
                ...
                this.tvwSpecimenBunryu.Width = SpecimenHangmogTreeMaxWidth;
            ...
            else
            {
                ...
                this.grdSpecimen.AutoSizeColumn(2, this.SpecimenHangmogNameNormalWidth);
                ...
                this.tvwSpecimenBunryu.Width = SpecimenHangmogTreeNormalWidth;
        ...
        private void btnSelect_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                grid = this.grdSearchOrder as XGrid;
                rownumber = this.grdSearchOrder.CurrentRowNumber;
            ...
            if (control.Checked)
            {
                ...
                switch (control.Name)
                {
                    case "rbnSearchOrder":   // 오더검색
                        ...
                        this.pnlSpecimen.Visible = false;
                        this.xLabel1.Visible = false;
                        this.cbxOutCommon.Visible = false;
                        ...
                        if (searchVoc != "")
                        {
                            if (this.rbnSearchOrder.Checked)
                            {
                                ...
                                this.LoadSearchOrderInfo(searchVoc, this.mOrderDate, INPUTTAB, wonnaeDrgYn);
                            }
                        }
                    ...
                    case "rbnCommon":   // 오더검색
                        ...
                        this.pnlSangyong.Visible = false;
                        this.pnlSpecimen.Visible = false;
                        this.xLabel1.Visible = true;
                        this.cbxOutCommon.Visible = true;
                        ...
                        if (this.rbnCommon.Checked)
                        {
                            //this.LoadSearchOrderInfo(searchVoc, this.dpkOrder_Date.GetDataValue(), INPUTTAB, wonnaeDrgYn);
                            this.LoadCommonOrderInfo(searchVoc, this.mOrderDate, INPUTTAB, wonnaeDrgYn);
                        }
                    ...
                    case "rbnSpecimen":       // 효능별

                        this.pnlSearch.Visible = false;
                        this.pnlSangyong.Visible = false;
                        this.pnlSpecimen.Visible = true;
                        this.xLabel1.Visible = false;
                        this.cbxOutCommon.Visible = false;
                        this.grdSpecimen.Reset();
                        ...
                        if (tvwSpecimenBunryu.Nodes[0].Text == "セット検査")
                            ...
                            this.tvwSpecimenBunryu.SelectedNode = (tvwSpecimenBunryu.Nodes[0].Nodes.Count > 1) ? tvwSpecimenBunryu.Nodes[0].Nodes[1] : tvwSpecimenBunryu.Nodes[0].Nodes[0];

                        PostCallHelper.PostCall(new PostMethod(PostRadioCheckedChanged));
                        //insert by jc TABが変わる際に検索語が既にあった場合はその検索語で検索する。
                        //if (searchVoc != "")
                        //{
                        if (this.rbnSpecimen.Checked)
                        {
                            if (searchVoc == "")
                            {
                                //if(this.tvwSpecimenBunryu.SelectedNode.Tag.ToString() == "System.Collections.Hashtable")
                                this.LoadSpecimenOrder("1", (Hashtable)this.tvwSpecimenBunryu.SelectedNode.Tag, wonnaeDrgYn, searchVoc);
                                //else
                                //    this.LoadSpecimenOrder("1", this.tvwSpecimenBunryu.SelectedNode.Tag.ToString(), wonnaeDrgYn, searchVoc);
                            }
                            else
                            {
                                this.LoadSpecimenOrder("2", "%", wonnaeDrgYn, searchVoc);
                            }
                            PostCallHelper.PostCall(new PostMethod(this.specimenChangedCheck));
        ...
        private void grdOrder_GridColumnChanged(object sender, GridColumnChangedEventArgs e)
        {
            ...
            switch (e.ColName)
            {
                case "hangmog_code":    // 오더코드 
                    ...
                    {
                        if (this.mHangmogInfo.LoadHangmogInfo(HangmogInfo.ExecutiveMode.CodeInput, loadExtraInfo, true) == false)
                        {
                            this.mOrderFunc.UndoPreviousValue(grid, e.RowNumber, e.ColName, previousValue);
                            this.OpenScreen_OCS0103Q00(e.ChangeValue.ToString(), true);
                            return;
        ...
        private void grdOrder_DragDrop(object sender, DragEventArgs e)
        {
            ...
            switch (gubun)
            {
                case "Specimen":
                    ...
                    hangmogCode = this.grdSpecimen.GetItemString(int.Parse(sourceRow), "hangmog_code");

                    //insert by jc 重複チェック　START
                    if (this.chkIsCanCreateOrder(this.grdSpecimen))
                    {
                ...
                case "Search":
                    ...
                    //insert by jc 重複チェック　START
                    if (this.chkIsCanCreateOrder(this.grdSearchOrder))
                    {
                        this.ApplySangOrderInfo(hangmogCode, rowNumber, HangmogInfo.ExecutiveMode.CodeInput, false);
        ...
        private void Search_text()
        {
            ...
            if (this.rbnSearchOrder.Checked)
            {
                this.LoadSearchOrderInfo(searchText, this.mOrderDate, INPUTTAB, wonnaeDrgYn);
                WarningMessage(grdSearchOrder);
            }
            else if (this.rbnCommon.Checked)
            {
                this.LoadCommonOrderInfo(searchText, this.mOrderDate, INPUTTAB, wonnaeDrgYn);
                WarningMessage(grdSearchOrder);
            }
            else if (this.rbnOftenOrder.Checked)
            {
            ...
            {
                //子が選択されてない状態であれば親の子の自動選択されるように
                if (this.tvwSpecimenBunryu.SelectedNode.Parent == null)
                    this.tvwSpecimenBunryu.SelectedNode = this.tvwSpecimenBunryu.SelectedNode.FirstNode;

                this.LoadSpecimenOrder("2", "%", wonnaeDrgYn, searchText);
                WarningMessage(grdSpecimen);
            }
        ...
        private void tvwSpecimenBunryu_MouseDown(object sender, MouseEventArgs e)
        {
            XEditGrid grid = this.grdSpecimen;
            //int rowNumber = -1;
            Image image_num = null;
            ArrayList deleteRow = new ArrayList();
            string group_number = this.getSelectedGroupNumber();
            ...
            if (e.Clicks == 2 && this.tvwSpecimenBunryu.SelectedNode.Level == 1 && (this.tvwSpecimenBunryu.SelectedNode.Parent.Text == "セット検査" || this.tvwSpecimenBunryu.SelectedNode.Parent.Text == "セット検査(個人)"))
            {

                string CurrStatus = "N";
                for (int ck = 0; ck < grid.RowCount; ck++)
                {
                    if (this.grdSpecimen.GetItemString(ck, "select_yn") == "Y")
                    {
                        CurrStatus = "Y";
                        break;
                    }
        ...
        private IList<object[]> GetGrdSpecimenList(BindVarCollection bvc)
        {
            ...
            OCS0103U13GrdSpecimenListResult res = CloudService.Instance.Submit<OCS0103U13GrdSpecimenListResult, OCS0103U13GrdSpecimenListArgs>(args);
        ...
        private void GetListCombo()
        {
            ...
            /*//Old Code
            res = CacheService.Instance.Get<OCS0103U13CboListArgs, OCS0103U13CboListResult>(arg, delegate(OCS0103U13CboListResult r)
            {
                return r.ExecutionStatus == ExecutionStatus.Success && (r.NalsuCboItem.Count > 0 || r.OrderGubunBasCboItem.Count > 0 || r.SearchConditionCboItem.Count > 0 || r.SuryangCboItem.Count > 0);
            });*/
        ...
        #region ExecuteLoadCommonOrderInfo
        /// <summary>
        /// ExecuteLoadSearchOrderInfo
        /// </summary>
        /// <param name="bvc"></param>
        /// <returns></returns>
        private IList<object[]> ExecuteLoadCommonOrderInfo(BindVarCollection bvc)
        {
            IList<object[]> lObj = new List<object[]>();
            ...
            args.Offset = "200";
            args.ProtocolId = protocolId;
            args.JundalTableOut = "CPL";
            LoadSearchCommonOrderResult result =
                CloudService.Instance.Submit<LoadSearchCommonOrderResult, LoadSearchCommonOrderArgs>(args);
            if (result.ExecutionStatus == ExecutionStatus.Success)
            {
                List<LoadSearchOrderInfo> listInfos = result.SearchResult;
                if (listInfos != null && listInfos.Count > 0)
        ...
        private void cbxOutCommon_CheckedChanged(object sender, EventArgs e)
        {
            ApplyDoYNOrderInfo(cbxOutCommon.Checked);
        }

        //append do_yn value to grdOrder
        private void ApplyDoYNOrderInfo(bool isCheck)
        {
            try
            {
                if (grdOrder.RowCount > 0)
                {
                    ...
                    for (int i = 0; i < grdOrder.RowCount; i++)
                    {
                        if (this.grdOrder.GetItemString(i, "group_ser") == tabInfo["group_ser"].ToString())
                        {
                            string doYn = isCheck ? "N" : "Y";
                            this.grdOrder.SetItemValue(i, "action_do_yn", doYn);
                        }
                    }
                }
            }
