...
set tagUrl=%repoRoot%/src/03.tags/cloud/%releaseVersion%
echo 'revision = %revision%'			
echo 'sourceURL = %url%'			
echo 'tagURL = %tagUrl%'
set home=.\
call svn copy -r %revision% -m "Tagging %releaseVersion% from %relativeUrl%, revision %revision%" %url% %tagUrl%
call powershell -Command "(gc %home%\pom.xml) -replace '<!--<module>med-proto</module>-->', '<module>med-proto</module>' | Out-File -encoding ASCII %home%\pom.xml"
