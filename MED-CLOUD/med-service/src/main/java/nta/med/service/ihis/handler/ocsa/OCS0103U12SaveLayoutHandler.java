...
import nta.med.service.ihis.proto.OcsaServiceProto.OCS0103U12SaveLayoutRequest;
...
@Service                                                                                                          
@Scope("prototype")                                                                                 
public class OCS0103U12SaveLayoutHandler 
	extends ScreenHandler<OcsaServiceProto.OCS0103U12SaveLayoutRequest, OcsaServiceProto.OCS0103U10SaveLayoutResponse> {                     
	private static final Log LOGGER = LogFactory.getLog(OCS0103U12SaveLayoutHandler.class);                                    
	@Resource                                                                                                       
	...
	public OCS0103U10SaveLayoutResponse handle(Vertx vertx, String clientId,
			...
			OCS0103U12SaveLayoutRequest request) {
		...
		try
		{
			...
			String ioFlag="";
			result = executeIud(request, hospCode);
			...
			for(PrOcsInterfaceInsertInfo item : request.getInterfaceInsertItemList()){
				...
				LOGGER.info("callPrOcsInterfaceInsert result="+ prOcsresult);
			}
			...
			if (!StringUtils.isEmpty(request.getOrderDate()) && DateUtil.toDate(request.getOrderDate(), DateUtil.PATTERN_YYMMDD) == null) {
				LOGGER.info("OrderDate is "+ request.getOrderDate() + " --> wrong format ");
	...
	private Integer executeIud(OcsaServiceProto.OCS0103U12SaveLayoutRequest request, String hospCode) {
		Integer result = null;
		...
		try {
			...
			if(!CollectionUtils.isEmpty(listOcs2003)) {
				for(Ocs2003 ocs2003 : listOcs2003) {
					...
					ocs2003.setSpecimenCode(info.getSpecimenCode());
					ocs2003.setBogyongCodeSub(info.getBogyongCodeSub());
				}
			}
