 ...
 * @param
 */
function MssScheduler(a) {
    a || (a = {});
    
    this.selectedData = void 0;
    this.doctors = a.doctors;
    MssScheduler.superclass_.constructor.apply(this, arguments);    
};
Util.inherit(MssScheduler, BaseScheduler);

MssScheduler.prototype.initDiffColumn = function(){
	this.columns = [{
...
MssScheduler.prototype.createTable = function(){
    return new Table({
...
MssScheduler.prototype.createTableRow = function(){
	...
	for(var i = 0; d <= this.endDate; i++){
		for(var j = 0; j < l; j++){
			...
			var r = new SchedulerRow(key, dt, this.timePeriods, isFirst = j==0?true:false);
...
MssScheduler.prototype.onCellDataChange = function(dt){
