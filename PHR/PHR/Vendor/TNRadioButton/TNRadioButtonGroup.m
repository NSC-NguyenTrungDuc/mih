...
#pragma mark - TNRadioButtonDelegate methods
- (void)radioButtonDidChangeWithIdentifier:(NSString*)identifier {
    
    for (TNRadioButton *rb in self.radioButtons) {
        
        if([rb.data.identifier isEqualToString:identifier]){
            rb.data.selected = true;
             self.selectedRadioButton = rb;
        } else {
            rb.data.selected = false;
        }
        [rb selectWithAnimation:YES];
    }
}
