...
public class ChartYAxisRenderer: ChartAxisRendererBase
{
    ...
    /// @QuyTM : Add to draw a rectangle base on [min] and [max] value and [color] depend on Axis's value
    /// Draws the rangeView at the specified position.
    public override func renderRangeView(context context: CGContext)
    {
        if (yAxis!.drawRangeViewColor == true) {
            var positionMax = CGPoint(x: 0.0, y: yAxis!.rangeViewMax)
            var positionMin = CGPoint(x: 0.0, y: yAxis!.rangeViewMin)
            transformer.pointValueToPixel(&positionMax)
            transformer.pointValueToPixel(&positionMin)
            
            guard let
                yAxis = yAxis,
                zeroLineColor = yAxis.rangeViewColor
                else { return }
            
            CGContextSaveGState(context)
            CGContextSetFillColorWithColor(context, zeroLineColor.CGColor)
            CGContextSetLineWidth(context, 0.5)
            CGContextMoveToPoint(context, viewPortHandler.contentLeft, positionMin.y)
            CGContextAddLineToPoint(context, viewPortHandler.contentLeft, positionMax.y)
            CGContextAddLineToPoint(context, viewPortHandler.contentRight, positionMax.y)
            CGContextAddLineToPoint(context, viewPortHandler.contentRight, positionMin.y)
            CGContextAddLineToPoint(context, viewPortHandler.contentLeft, positionMin.y)
    ...
    public override func renderLimitLines(context context: CGContext)
    {
        ...
        for i in 0 ..< limitLines.count
        {
            ...
            //TungNT
            let label = l.label
            var labelLineWidth = CGSizeZero
            if (label.characters.count > 0){
                let fontAttributes = [NSFontAttributeName: l.valueFont]
                labelLineWidth = (l.label as NSString).sizeWithAttributes(fontAttributes)
            }
            ...
            if (l.labelPosition == .Left) {
                _limitLineSegmentsBuffer[0].x = viewPortHandler.contentLeft + labelLineWidth.width + 4.0 + l.xOffset + 15.0
            } else {
            ...
            if (label.characters.count > 0)
            {
                ...
                else if (l.labelPosition == .LeftTop)
                {
                    ChartUtils.drawText(context: context,
                                        ...
                                        align: .Left,
                                        attributes: [NSFontAttributeName: l.valueFont, NSForegroundColorAttributeName: l.valueTextColor])
                } else if (l.labelPosition == .Left) //TungNT
                {
                    ChartUtils.drawTextWithIcon(context: context,
                                                text: label,
                                                point: CGPoint(
                                                    x: viewPortHandler.contentLeft + xOffset + (labelLineWidth as CGSize).width + 10,
