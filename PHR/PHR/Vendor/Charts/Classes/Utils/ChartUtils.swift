...
public class ChartUtils
{
    ...
    //TungNT
    public class func drawTextWithIcon(context context: CGContext, text: String, point: CGPoint, align: NSTextAlignment, attributes: [String : AnyObject]?)
    {
        var point = point
        ...
        let imageName = "Icon_Bell_2"
        let image = UIImage(named: imageName)
        
        let pointImage = CGPoint(x: point.x - (image! as UIImage).size.width, y: point.y)
        
        image?.drawAtPoint(pointImage);
