...
import nta.med.ext.phr.glossary.DateTimeFormat;
 ...
 * @CrtDate Jul 23, 2014
 */
public class MedDateTimeUtil {
	
	/** The Constant LOG. */
	private static final Logger LOG = LogManager.getLogger(MedDateTimeUtil.class);
	...
	public static Date dateFromString(String dateString, DateTimeFormat format) {
		...
		try {
			...
			LOG.error("Error when parsing string to date.", e);
	...
	public static String dateToString(Date date, DateTimeFormat format) {
		...
		try {
			...
			LOG.error("Error when format date to string.", e);
			LOG.error("Date: ", date);
			LOG.error("Format: ", format);
		}
		return dateString;
