(function () {
    ...
    angular
        .module('app')
        .controller('PatientSurveyBranchingController', PatientSurveyBranchingController);

    PatientSurveyBranchingController.$inject = ['$document', '$rootScope', '$route', '$scope', '$routeParams', 'cssInjector', '$location', 'PatientSurveysService', 'DTOptionsBuilder', '$compile', 'DTColumnDefBuilder', 'gettextCatalog', '$http', '$timeout', 'localStorageService', '$cookieStore', '$interval', 'PatientService'];
    function PatientSurveyBranchingController($document, $rootScope, $route, $scope, $routeParams, cssInjector, $location, PatientSurveysService, DTOptionsBuilder, $compile, DTColumnDefBuilder, gettextCatalog, $http, $timeout, localStorageService, $cookieStore, $interval, PatientService) {
        ...
        function initController() {
            ...
            $http.get(url).then(function (res) {
                ...
                if ($location.path() == '/patient/do-survey-only-mt-branching/' + $routeParams.patient_sid) { // For do survey page
                    ...
                    PatientSurveysService.doSurvey($routeParams.patient_sid).then(function (res) {
                        if (res.status == 1) {
                            ...
                            if(res.data.survey_information != undefined && res.data.survey_information.survey_type != undefined && res.data.survey_information.survey_type == 1) {
                                $location.path('/patient/do-survey-only-mt/' + $routeParams.patient_sid);
                            }
                ...
                if (listQuestion.length > 0) {
                    ...
                    $.each($scope.surveyObj.survey_information.question_group[indexGroup].question_list, function (index_question, value_question) {
                        if (value_question.question_id == listQuestion_id) {
                            ...
                            question = value_question;
                            listQuestion.splice(listQuestion.indexOf(value_question.question_id), 1);
                            indexQuestionList = index_question;
                ...
                $.each($scope.surveyObj.survey_information.question_group, function (index, value) {
                    $.each(value.question_list, function (idx, val) {
                        if (val.question_id == preview_question_id) {
                            ...
                            $.each(listAnsweredTemp,function (idx_lst,val_lst) {
                                if(val_lst.index == preview_question_id)
                                {
                                    $scope.listAnswered.splice(idx_lst,1);
                                }
                            })
                        }
                    });
                        ...
                        footer: function (currentPage, pageCount) {
                            return {
                                stack: [
                                    ...
                                    {
                                        text: (hospAddKey == null) ? ' ' : hospAddKey,
                                        fontSize: 10,
                                        alignment: 'right',
                                        margin: [0, 0, 30, 0]
                                    }
