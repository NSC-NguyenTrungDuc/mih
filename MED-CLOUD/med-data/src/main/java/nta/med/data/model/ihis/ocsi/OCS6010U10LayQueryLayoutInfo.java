package nta.med.data.model.ihis.ocsi;

import java.util.Date;

public class OCS6010U10LayQueryLayoutInfo {

	private Double inOutKey;
	private Double pkocskey;
	private String bunho;
	private String orderDate;
	private String gwa;
	private String doctor;
	private String resident;
	private String naewonType;
	private String jubsuNo;
	private String inputId;
	private String inputPart;
	private String inputGwa;
	private String inputDoctor;
	private String inputGubun;
	private String inputGubunName;
	private Double groupSer;
	private String inputTab;
	private String inputTabName;
	private String orderGubun;
	private String orderGubunName;
	private String groupYn;
	private Double seq;
	private String slipCode;
	private String hangmogCode;
	private String hangmogName;
	private String specimenCode;
	private String specimenName;
	private Double suryang;
	private Double sunabSuryang;
	private Double subulSuryang;
	private String ordDanui;
	private String ordDanuiName;
	private String dvTime;
	private Double dv;
	private Double dv1;
	private Double dv2;
	private Double dv3;
	private Double dv4;
	private Double nalsu;
	private String sunabNalsu;
	private String jusa;
	private String jusaName;
	private String jusaSpdGubun;
	private String bogyongCode;
	private String bogyongName;
	private String emergency;
	private String jaeryoJundalYn;
	private String jundalTable;
	private String jundalPart;
	private String movePart;
	private String portableYn1;
	private String powderYn;
	private String hubalChangeYn;
	private String pharmacy;
	private String drgPackYn;
	private String muhyo;
	private String prnYn;
	private String toiwonDrgYn;
	private String prnNurse;
	private String appendYn;
	private String orderRemark;
	private String nurseRemark;
	private String comments;
	private String mixGroup;
	private Double amt;
	private String pay;
	private String wonyoiOrderYn;
	private String dangilGsOrder;
	private String dangilGsResult;
	private String bomOccurYn;
	private Double bomSourceKey;
	private String displayYn;
	private String sunabYn;
	private String sunabDate;
	private String sunabTime;
	private String hopeDate;
	private String hopeTime;
	private String nurseConfUser;
	private String nurseConfDate;
	private String nurseConfTime;
	private String nursePickupUser;
	private String nursePickupDate;
	private String nursePickupTime;
	private String nurseHoldUser;
	private String nurseHoldDate;
	private String nurseHoldTime;
	private String reserDate;
	private String reserTime;
	private String jubsuDate;
	private String jubsuTime;
	private String actingDate;
	private String actingTime;
	private Double actingDay;
	private String resultDate;
	private String dcGubun;
	private String dcYn;
	private String bannabYn;
	private String bannabConfirm;
	private Double sourceOrdKey;
	private String ocsFlag;
	private String sgCode;
	private Date sgYmd;
	private String ioGubun;
	private String afterActYn;
	private String bichiYn;
	private String drgBunho;
	private String subSusul;
	private String printYn;
	private String chisik;
	private String telYn;
	private String orderGubunBas;
	private String inputControl;
	private String sugaYn;
	private String jaeryoYn;
	private String wonyoiCheck;
	private String emergencyCheck;
	private String specimenCheck;
	private String portableYn2;
	private String bulyongCheck;
	private String sunabCheck;
	private String dcCheck;
	private String dcGubunCheck;
	private String confirmCheck;
	private String reserYnCheck;
	private String chisikYn;
	private String ndayYn;
	private String defltJaeryoJundal;
	private String defltWonyoiYn;
	private String specCmt;
	private String specCmtName;
	private String specCmtSysId;
	private String specCmtPgmId;
	private String specCmtNotNull;
	private String specCmtTblId;
	private String specCmtColId;
	private String donbogYn;
	private String ordGubBasName;
	private String actDoctor;
	private String actBuseo;
	private String actGwa;
	private String homeCareYn;
	private String regularYn;
	private Double sortFkocskey;
	private String childYn;
	private String ifInputCtrl;
	private String childGubun;
	private String slipName;
	private String ndayOccurYn;
	private Double orgKey;
	private Double parentKey;
	private String bunCode;
	private String wonnaeDrgYn;
	private String hubalChangeCk;
	private String drgPackCheck;
	private String pharmacyCheck;
	private String powderCheck;
	private String orderByKey;

	public OCS6010U10LayQueryLayoutInfo(Double inOutKey, Double pkocskey, String bunho, String orderDate, String gwa,
			String doctor, String resident, String naewonType, String jubsuNo, String inputId, String inputPart,
			String inputGwa, String inputDoctor, String inputGubun, String inputGubunName, Double groupSer,
			String inputTab, String inputTabName, String orderGubun, String orderGubunName, String groupYn, Double seq,
			String slipCode, String hangmogCode, String hangmogName, String specimenCode, String specimenName,
			Double suryang, Double sunabSuryang, Double subulSuryang, String ordDanui, String ordDanuiName,
			String dvTime, Double dv, Double dv1, Double dv2, Double dv3, Double dv4, Double nalsu, String sunabNalsu,
			String jusa, String jusaName, String jusaSpdGubun, String bogyongCode, String bogyongName, String emergency,
			String jaeryoJundalYn, String jundalTable, String jundalPart, String movePart, String portableYn1,
			String powderYn, String hubalChangeYn, String pharmacy, String drgPackYn, String muhyo, String prnYn,
			String toiwonDrgYn, String prnNurse, String appendYn, String orderRemark, String nurseRemark,
			String comments, String mixGroup, Double amt, String pay, String wonyoiOrderYn, String dangilGsOrder,
			String dangilGsResult, String bomOccurYn, Double bomSourceKey, String displayYn, String sunabYn,
			String sunabDate, String sunabTime, String hopeDate, String hopeTime, String nurseConfUser,
			String nurseConfDate, String nurseConfTime, String nursePickupUser, String nursePickupDate,
			String nursePickupTime, String nurseHoldUser, String nurseHoldDate, String nurseHoldTime, String reserDate,
			String reserTime, String jubsuDate, String jubsuTime, String actingDate, String actingTime,
			Double actingDay, String resultDate, String dcGubun, String dcYn, String bannabYn, String bannabConfirm,
			Double sourceOrdKey, String ocsFlag, String sgCode, Date sgYmd, String ioGubun, String afterActYn,
			String bichiYn, String drgBunho, String subSusul, String printYn, String chisik, String telYn,
			String orderGubunBas, String inputControl, String sugaYn, String jaeryoYn, String wonyoiCheck,
			String emergencyCheck, String specimenCheck, String portableYn2, String bulyongCheck, String sunabCheck,
			String dcCheck, String dcGubunCheck, String confirmCheck, String reserYnCheck, String chisikYn,
			String ndayYn, String defltJaeryoJundal, String defltWonyoiYn, String specCmt, String specCmtName,
			String specCmtSysId, String specCmtPgmId, String specCmtNotNull, String specCmtTblId, String specCmtColId,
			String donbogYn, String ordGubBasName, String actDoctor, String actBuseo, String actGwa, String homeCareYn,
			String regularYn, Double sortFkocskey, String childYn, String ifInputCtrl, String childGubun,
			String slipName, String ndayOccurYn, Double orgKey, Double parentKey, String bunCode, String wonnaeDrgYn,
			String hubalChangeCk, String drgPackCheck, String pharmacyCheck, String powderCheck, String orderByKey) {
		super();
		this.inOutKey = inOutKey;
		this.pkocskey = pkocskey;
		this.bunho = bunho;
		this.orderDate = orderDate;
		this.gwa = gwa;
		this.doctor = doctor;
		this.resident = resident;
		this.naewonType = naewonType;
		this.jubsuNo = jubsuNo;
		this.inputId = inputId;
		this.inputPart = inputPart;
		this.inputGwa = inputGwa;
		this.inputDoctor = inputDoctor;
		this.inputGubun = inputGubun;
		this.inputGubunName = inputGubunName;
		this.groupSer = groupSer;
		this.inputTab = inputTab;
		this.inputTabName = inputTabName;
		this.orderGubun = orderGubun;
		this.orderGubunName = orderGubunName;
		this.groupYn = groupYn;
		this.seq = seq;
		this.slipCode = slipCode;
		this.hangmogCode = hangmogCode;
		this.hangmogName = hangmogName;
		this.specimenCode = specimenCode;
		this.specimenName = specimenName;
		this.suryang = suryang;
		this.sunabSuryang = sunabSuryang;
		this.subulSuryang = subulSuryang;
		this.ordDanui = ordDanui;
		this.ordDanuiName = ordDanuiName;
		this.dvTime = dvTime;
		this.dv = dv;
		this.dv1 = dv1;
		this.dv2 = dv2;
		this.dv3 = dv3;
		this.dv4 = dv4;
		this.nalsu = nalsu;
		this.sunabNalsu = sunabNalsu;
		this.jusa = jusa;
		this.jusaName = jusaName;
		this.jusaSpdGubun = jusaSpdGubun;
		this.bogyongCode = bogyongCode;
		this.bogyongName = bogyongName;
		this.emergency = emergency;
		this.jaeryoJundalYn = jaeryoJundalYn;
		this.jundalTable = jundalTable;
		this.jundalPart = jundalPart;
		this.movePart = movePart;
		this.portableYn1 = portableYn1;
		this.powderYn = powderYn;
		this.hubalChangeYn = hubalChangeYn;
		this.pharmacy = pharmacy;
		this.drgPackYn = drgPackYn;
		this.muhyo = muhyo;
		this.prnYn = prnYn;
		this.toiwonDrgYn = toiwonDrgYn;
		this.prnNurse = prnNurse;
		this.appendYn = appendYn;
		this.orderRemark = orderRemark;
		this.nurseRemark = nurseRemark;
		this.comments = comments;
		this.mixGroup = mixGroup;
		this.amt = amt;
		this.pay = pay;
		this.wonyoiOrderYn = wonyoiOrderYn;
		this.dangilGsOrder = dangilGsOrder;
		this.dangilGsResult = dangilGsResult;
		this.bomOccurYn = bomOccurYn;
		this.bomSourceKey = bomSourceKey;
		this.displayYn = displayYn;
		this.sunabYn = sunabYn;
		this.sunabDate = sunabDate;
		this.sunabTime = sunabTime;
		this.hopeDate = hopeDate;
		this.hopeTime = hopeTime;
		this.nurseConfUser = nurseConfUser;
		this.nurseConfDate = nurseConfDate;
		this.nurseConfTime = nurseConfTime;
		this.nursePickupUser = nursePickupUser;
		this.nursePickupDate = nursePickupDate;
		this.nursePickupTime = nursePickupTime;
		this.nurseHoldUser = nurseHoldUser;
		this.nurseHoldDate = nurseHoldDate;
		this.nurseHoldTime = nurseHoldTime;
		this.reserDate = reserDate;
		this.reserTime = reserTime;
		this.jubsuDate = jubsuDate;
		this.jubsuTime = jubsuTime;
		this.actingDate = actingDate;
		this.actingTime = actingTime;
		this.actingDay = actingDay;
		this.resultDate = resultDate;
		this.dcGubun = dcGubun;
		this.dcYn = dcYn;
		this.bannabYn = bannabYn;
		this.bannabConfirm = bannabConfirm;
		this.sourceOrdKey = sourceOrdKey;
		this.ocsFlag = ocsFlag;
		this.sgCode = sgCode;
		this.sgYmd = sgYmd;
		this.ioGubun = ioGubun;
		this.afterActYn = afterActYn;
		this.bichiYn = bichiYn;
		this.drgBunho = drgBunho;
		this.subSusul = subSusul;
		this.printYn = printYn;
		this.chisik = chisik;
		this.telYn = telYn;
		this.orderGubunBas = orderGubunBas;
		this.inputControl = inputControl;
		this.sugaYn = sugaYn;
		this.jaeryoYn = jaeryoYn;
		this.wonyoiCheck = wonyoiCheck;
		this.emergencyCheck = emergencyCheck;
		this.specimenCheck = specimenCheck;
		this.portableYn2 = portableYn2;
		this.bulyongCheck = bulyongCheck;
		this.sunabCheck = sunabCheck;
		this.dcCheck = dcCheck;
		this.dcGubunCheck = dcGubunCheck;
		this.confirmCheck = confirmCheck;
		this.reserYnCheck = reserYnCheck;
		this.chisikYn = chisikYn;
		this.ndayYn = ndayYn;
		this.defltJaeryoJundal = defltJaeryoJundal;
		this.defltWonyoiYn = defltWonyoiYn;
		this.specCmt = specCmt;
		this.specCmtName = specCmtName;
		this.specCmtSysId = specCmtSysId;
		this.specCmtPgmId = specCmtPgmId;
		this.specCmtNotNull = specCmtNotNull;
		this.specCmtTblId = specCmtTblId;
		this.specCmtColId = specCmtColId;
		this.donbogYn = donbogYn;
		this.ordGubBasName = ordGubBasName;
		this.actDoctor = actDoctor;
		this.actBuseo = actBuseo;
		this.actGwa = actGwa;
		this.homeCareYn = homeCareYn;
		this.regularYn = regularYn;
		this.sortFkocskey = sortFkocskey;
		this.childYn = childYn;
		this.ifInputCtrl = ifInputCtrl;
		this.childGubun = childGubun;
		this.slipName = slipName;
		this.ndayOccurYn = ndayOccurYn;
		this.orgKey = orgKey;
		this.parentKey = parentKey;
		this.bunCode = bunCode;
		this.wonnaeDrgYn = wonnaeDrgYn;
		this.hubalChangeCk = hubalChangeCk;
		this.drgPackCheck = drgPackCheck;
		this.pharmacyCheck = pharmacyCheck;
		this.powderCheck = powderCheck;
		this.orderByKey = orderByKey;
	}

	public Double getInOutKey() {
		return inOutKey;
	}

	public void setInOutKey(Double inOutKey) {
		this.inOutKey = inOutKey;
	}

	public Double getPkocskey() {
		return pkocskey;
	}

	public void setPkocskey(Double pkocskey) {
		this.pkocskey = pkocskey;
	}

	public String getBunho() {
		return bunho;
	}

	public void setBunho(String bunho) {
		this.bunho = bunho;
	}

	public String getOrderDate() {
		return orderDate;
	}

	public void setOrderDate(String orderDate) {
		this.orderDate = orderDate;
	}

	public String getGwa() {
		return gwa;
	}

	public void setGwa(String gwa) {
		this.gwa = gwa;
	}

	public String getDoctor() {
		return doctor;
	}

	public void setDoctor(String doctor) {
		this.doctor = doctor;
	}

	public String getResident() {
		return resident;
	}

	public void setResident(String resident) {
		this.resident = resident;
	}

	public String getNaewonType() {
		return naewonType;
	}

	public void setNaewonType(String naewonType) {
		this.naewonType = naewonType;
	}

	public String getJubsuNo() {
		return jubsuNo;
	}

	public void setJubsuNo(String jubsuNo) {
		this.jubsuNo = jubsuNo;
	}

	public String getInputId() {
		return inputId;
	}

	public void setInputId(String inputId) {
		this.inputId = inputId;
	}

	public String getInputPart() {
		return inputPart;
	}

	public void setInputPart(String inputPart) {
		this.inputPart = inputPart;
	}

	public String getInputGwa() {
		return inputGwa;
	}

	public void setInputGwa(String inputGwa) {
		this.inputGwa = inputGwa;
	}

	public String getInputDoctor() {
		return inputDoctor;
	}

	public void setInputDoctor(String inputDoctor) {
		this.inputDoctor = inputDoctor;
	}

	public String getInputGubun() {
		return inputGubun;
	}

	public void setInputGubun(String inputGubun) {
		this.inputGubun = inputGubun;
	}

	public String getInputGubunName() {
		return inputGubunName;
	}

	public void setInputGubunName(String inputGubunName) {
		this.inputGubunName = inputGubunName;
	}

	public Double getGroupSer() {
		return groupSer;
	}

	public void setGroupSer(Double groupSer) {
		this.groupSer = groupSer;
	}

	public String getInputTab() {
		return inputTab;
	}

	public void setInputTab(String inputTab) {
		this.inputTab = inputTab;
	}

	public String getInputTabName() {
		return inputTabName;
	}

	public void setInputTabName(String inputTabName) {
		this.inputTabName = inputTabName;
	}

	public String getOrderGubun() {
		return orderGubun;
	}

	public void setOrderGubun(String orderGubun) {
		this.orderGubun = orderGubun;
	}

	public String getOrderGubunName() {
		return orderGubunName;
	}

	public void setOrderGubunName(String orderGubunName) {
		this.orderGubunName = orderGubunName;
	}

	public String getGroupYn() {
		return groupYn;
	}

	public void setGroupYn(String groupYn) {
		this.groupYn = groupYn;
	}

	public Double getSeq() {
		return seq;
	}

	public void setSeq(Double seq) {
		this.seq = seq;
	}

	public String getSlipCode() {
		return slipCode;
	}

	public void setSlipCode(String slipCode) {
		this.slipCode = slipCode;
	}

	public String getHangmogCode() {
		return hangmogCode;
	}

	public void setHangmogCode(String hangmogCode) {
		this.hangmogCode = hangmogCode;
	}

	public String getHangmogName() {
		return hangmogName;
	}

	public void setHangmogName(String hangmogName) {
		this.hangmogName = hangmogName;
	}

	public String getSpecimenCode() {
		return specimenCode;
	}

	public void setSpecimenCode(String specimenCode) {
		this.specimenCode = specimenCode;
	}

	public String getSpecimenName() {
		return specimenName;
	}

	public void setSpecimenName(String specimenName) {
		this.specimenName = specimenName;
	}

	public Double getSuryang() {
		return suryang;
	}

	public void setSuryang(Double suryang) {
		this.suryang = suryang;
	}

	public Double getSunabSuryang() {
		return sunabSuryang;
	}

	public void setSunabSuryang(Double sunabSuryang) {
		this.sunabSuryang = sunabSuryang;
	}

	public Double getSubulSuryang() {
		return subulSuryang;
	}

	public void setSubulSuryang(Double subulSuryang) {
		this.subulSuryang = subulSuryang;
	}

	public String getOrdDanui() {
		return ordDanui;
	}

	public void setOrdDanui(String ordDanui) {
		this.ordDanui = ordDanui;
	}

	public String getOrdDanuiName() {
		return ordDanuiName;
	}

	public void setOrdDanuiName(String ordDanuiName) {
		this.ordDanuiName = ordDanuiName;
	}

	public String getDvTime() {
		return dvTime;
	}

	public void setDvTime(String dvTime) {
		this.dvTime = dvTime;
	}

	public Double getDv() {
		return dv;
	}

	public void setDv(Double dv) {
		this.dv = dv;
	}

	public Double getDv1() {
		return dv1;
	}

	public void setDv1(Double dv1) {
		this.dv1 = dv1;
	}

	public Double getDv2() {
		return dv2;
	}

	public void setDv2(Double dv2) {
		this.dv2 = dv2;
	}

	public Double getDv3() {
		return dv3;
	}

	public void setDv3(Double dv3) {
		this.dv3 = dv3;
	}

	public Double getDv4() {
		return dv4;
	}

	public void setDv4(Double dv4) {
		this.dv4 = dv4;
	}

	public Double getNalsu() {
		return nalsu;
	}

	public void setNalsu(Double nalsu) {
		this.nalsu = nalsu;
	}

	public String getSunabNalsu() {
		return sunabNalsu;
	}

	public void setSunabNalsu(String sunabNalsu) {
		this.sunabNalsu = sunabNalsu;
	}

	public String getJusa() {
		return jusa;
	}

	public void setJusa(String jusa) {
		this.jusa = jusa;
	}

	public String getJusaName() {
		return jusaName;
	}

	public void setJusaName(String jusaName) {
		this.jusaName = jusaName;
	}

	public String getJusaSpdGubun() {
		return jusaSpdGubun;
	}

	public void setJusaSpdGubun(String jusaSpdGubun) {
		this.jusaSpdGubun = jusaSpdGubun;
	}

	public String getBogyongCode() {
		return bogyongCode;
	}

	public void setBogyongCode(String bogyongCode) {
		this.bogyongCode = bogyongCode;
	}

	public String getBogyongName() {
		return bogyongName;
	}

	public void setBogyongName(String bogyongName) {
		this.bogyongName = bogyongName;
	}

	public String getEmergency() {
		return emergency;
	}

	public void setEmergency(String emergency) {
		this.emergency = emergency;
	}

	public String getJaeryoJundalYn() {
		return jaeryoJundalYn;
	}

	public void setJaeryoJundalYn(String jaeryoJundalYn) {
		this.jaeryoJundalYn = jaeryoJundalYn;
	}

	public String getJundalTable() {
		return jundalTable;
	}

	public void setJundalTable(String jundalTable) {
		this.jundalTable = jundalTable;
	}

	public String getJundalPart() {
		return jundalPart;
	}

	public void setJundalPart(String jundalPart) {
		this.jundalPart = jundalPart;
	}

	public String getMovePart() {
		return movePart;
	}

	public void setMovePart(String movePart) {
		this.movePart = movePart;
	}

	public String getPortableYn1() {
		return portableYn1;
	}

	public void setPortableYn1(String portableYn1) {
		this.portableYn1 = portableYn1;
	}

	public String getPowderYn() {
		return powderYn;
	}

	public void setPowderYn(String powderYn) {
		this.powderYn = powderYn;
	}

	public String getHubalChangeYn() {
		return hubalChangeYn;
	}

	public void setHubalChangeYn(String hubalChangeYn) {
		this.hubalChangeYn = hubalChangeYn;
	}

	public String getPharmacy() {
		return pharmacy;
	}

	public void setPharmacy(String pharmacy) {
		this.pharmacy = pharmacy;
	}

	public String getDrgPackYn() {
		return drgPackYn;
	}

	public void setDrgPackYn(String drgPackYn) {
		this.drgPackYn = drgPackYn;
	}

	public String getMuhyo() {
		return muhyo;
	}

	public void setMuhyo(String muhyo) {
		this.muhyo = muhyo;
	}

	public String getPrnYn() {
		return prnYn;
	}

	public void setPrnYn(String prnYn) {
		this.prnYn = prnYn;
	}

	public String getToiwonDrgYn() {
		return toiwonDrgYn;
	}

	public void setToiwonDrgYn(String toiwonDrgYn) {
		this.toiwonDrgYn = toiwonDrgYn;
	}

	public String getPrnNurse() {
		return prnNurse;
	}

	public void setPrnNurse(String prnNurse) {
		this.prnNurse = prnNurse;
	}

	public String getAppendYn() {
		return appendYn;
	}

	public void setAppendYn(String appendYn) {
		this.appendYn = appendYn;
	}

	public String getOrderRemark() {
		return orderRemark;
	}

	public void setOrderRemark(String orderRemark) {
		this.orderRemark = orderRemark;
	}

	public String getNurseRemark() {
		return nurseRemark;
	}

	public void setNurseRemark(String nurseRemark) {
		this.nurseRemark = nurseRemark;
	}

	public String getComments() {
		return comments;
	}

	public void setComments(String comments) {
		this.comments = comments;
	}

	public String getMixGroup() {
		return mixGroup;
	}

	public void setMixGroup(String mixGroup) {
		this.mixGroup = mixGroup;
	}

	public Double getAmt() {
		return amt;
	}

	public void setAmt(Double amt) {
		this.amt = amt;
	}

	public String getPay() {
		return pay;
	}

	public void setPay(String pay) {
		this.pay = pay;
	}

	public String getWonyoiOrderYn() {
		return wonyoiOrderYn;
	}

	public void setWonyoiOrderYn(String wonyoiOrderYn) {
		this.wonyoiOrderYn = wonyoiOrderYn;
	}

	public String getDangilGsOrder() {
		return dangilGsOrder;
	}

	public void setDangilGsOrder(String dangilGsOrder) {
		this.dangilGsOrder = dangilGsOrder;
	}

	public String getDangilGsResult() {
		return dangilGsResult;
	}

	public void setDangilGsResult(String dangilGsResult) {
		this.dangilGsResult = dangilGsResult;
	}

	public String getBomOccurYn() {
		return bomOccurYn;
	}

	public void setBomOccurYn(String bomOccurYn) {
		this.bomOccurYn = bomOccurYn;
	}

	public Double getBomSourceKey() {
		return bomSourceKey;
	}

	public void setBomSourceKey(Double bomSourceKey) {
		this.bomSourceKey = bomSourceKey;
	}

	public String getDisplayYn() {
		return displayYn;
	}

	public void setDisplayYn(String displayYn) {
		this.displayYn = displayYn;
	}

	public String getSunabYn() {
		return sunabYn;
	}

	public void setSunabYn(String sunabYn) {
		this.sunabYn = sunabYn;
	}

	public String getSunabDate() {
		return sunabDate;
	}

	public void setSunabDate(String sunabDate) {
		this.sunabDate = sunabDate;
	}

	public String getSunabTime() {
		return sunabTime;
	}

	public void setSunabTime(String sunabTime) {
		this.sunabTime = sunabTime;
	}

	public String getHopeDate() {
		return hopeDate;
	}

	public void setHopeDate(String hopeDate) {
		this.hopeDate = hopeDate;
	}

	public String getHopeTime() {
		return hopeTime;
	}

	public void setHopeTime(String hopeTime) {
		this.hopeTime = hopeTime;
	}

	public String getNurseConfUser() {
		return nurseConfUser;
	}

	public void setNurseConfUser(String nurseConfUser) {
		this.nurseConfUser = nurseConfUser;
	}

	public String getNurseConfDate() {
		return nurseConfDate;
	}

	public void setNurseConfDate(String nurseConfDate) {
		this.nurseConfDate = nurseConfDate;
	}

	public String getNurseConfTime() {
		return nurseConfTime;
	}

	public void setNurseConfTime(String nurseConfTime) {
		this.nurseConfTime = nurseConfTime;
	}

	public String getNursePickupUser() {
		return nursePickupUser;
	}

	public void setNursePickupUser(String nursePickupUser) {
		this.nursePickupUser = nursePickupUser;
	}

	public String getNursePickupDate() {
		return nursePickupDate;
	}

	public void setNursePickupDate(String nursePickupDate) {
		this.nursePickupDate = nursePickupDate;
	}

	public String getNursePickupTime() {
		return nursePickupTime;
	}

	public void setNursePickupTime(String nursePickupTime) {
		this.nursePickupTime = nursePickupTime;
	}

	public String getNurseHoldUser() {
		return nurseHoldUser;
	}

	public void setNurseHoldUser(String nurseHoldUser) {
		this.nurseHoldUser = nurseHoldUser;
	}

	public String getNurseHoldDate() {
		return nurseHoldDate;
	}

	public void setNurseHoldDate(String nurseHoldDate) {
		this.nurseHoldDate = nurseHoldDate;
	}

	public String getNurseHoldTime() {
		return nurseHoldTime;
	}

	public void setNurseHoldTime(String nurseHoldTime) {
		this.nurseHoldTime = nurseHoldTime;
	}

	public String getReserDate() {
		return reserDate;
	}

	public void setReserDate(String reserDate) {
		this.reserDate = reserDate;
	}

	public String getReserTime() {
		return reserTime;
	}

	public void setReserTime(String reserTime) {
		this.reserTime = reserTime;
	}

	public String getJubsuDate() {
		return jubsuDate;
	}

	public void setJubsuDate(String jubsuDate) {
		this.jubsuDate = jubsuDate;
	}

	public String getJubsuTime() {
		return jubsuTime;
	}

	public void setJubsuTime(String jubsuTime) {
		this.jubsuTime = jubsuTime;
	}

	public String getActingDate() {
		return actingDate;
	}

	public void setActingDate(String actingDate) {
		this.actingDate = actingDate;
	}

	public String getActingTime() {
		return actingTime;
	}

	public void setActingTime(String actingTime) {
		this.actingTime = actingTime;
	}

	public Double getActingDay() {
		return actingDay;
	}

	public void setActingDay(Double actingDay) {
		this.actingDay = actingDay;
	}

	public String getResultDate() {
		return resultDate;
	}

	public void setResultDate(String resultDate) {
		this.resultDate = resultDate;
	}

	public String getDcGubun() {
		return dcGubun;
	}

	public void setDcGubun(String dcGubun) {
		this.dcGubun = dcGubun;
	}

	public String getDcYn() {
		return dcYn;
	}

	public void setDcYn(String dcYn) {
		this.dcYn = dcYn;
	}

	public String getBannabYn() {
		return bannabYn;
	}

	public void setBannabYn(String bannabYn) {
		this.bannabYn = bannabYn;
	}

	public String getBannabConfirm() {
		return bannabConfirm;
	}

	public void setBannabConfirm(String bannabConfirm) {
		this.bannabConfirm = bannabConfirm;
	}

	public Double getSourceOrdKey() {
		return sourceOrdKey;
	}

	public void setSourceOrdKey(Double sourceOrdKey) {
		this.sourceOrdKey = sourceOrdKey;
	}

	public String getOcsFlag() {
		return ocsFlag;
	}

	public void setOcsFlag(String ocsFlag) {
		this.ocsFlag = ocsFlag;
	}

	public String getSgCode() {
		return sgCode;
	}

	public void setSgCode(String sgCode) {
		this.sgCode = sgCode;
	}

	public Date getSgYmd() {
		return sgYmd;
	}

	public void setSgYmd(Date sgYmd) {
		this.sgYmd = sgYmd;
	}

	public String getIoGubun() {
		return ioGubun;
	}

	public void setIoGubun(String ioGubun) {
		this.ioGubun = ioGubun;
	}

	public String getAfterActYn() {
		return afterActYn;
	}

	public void setAfterActYn(String afterActYn) {
		this.afterActYn = afterActYn;
	}

	public String getBichiYn() {
		return bichiYn;
	}

	public void setBichiYn(String bichiYn) {
		this.bichiYn = bichiYn;
	}

	public String getDrgBunho() {
		return drgBunho;
	}

	public void setDrgBunho(String drgBunho) {
		this.drgBunho = drgBunho;
	}

	public String getSubSusul() {
		return subSusul;
	}

	public void setSubSusul(String subSusul) {
		this.subSusul = subSusul;
	}

	public String getPrintYn() {
		return printYn;
	}

	public void setPrintYn(String printYn) {
		this.printYn = printYn;
	}

	public String getChisik() {
		return chisik;
	}

	public void setChisik(String chisik) {
		this.chisik = chisik;
	}

	public String getTelYn() {
		return telYn;
	}

	public void setTelYn(String telYn) {
		this.telYn = telYn;
	}

	public String getOrderGubunBas() {
		return orderGubunBas;
	}

	public void setOrderGubunBas(String orderGubunBas) {
		this.orderGubunBas = orderGubunBas;
	}

	public String getInputControl() {
		return inputControl;
	}

	public void setInputControl(String inputControl) {
		this.inputControl = inputControl;
	}

	public String getSugaYn() {
		return sugaYn;
	}

	public void setSugaYn(String sugaYn) {
		this.sugaYn = sugaYn;
	}

	public String getJaeryoYn() {
		return jaeryoYn;
	}

	public void setJaeryoYn(String jaeryoYn) {
		this.jaeryoYn = jaeryoYn;
	}

	public String getWonyoiCheck() {
		return wonyoiCheck;
	}

	public void setWonyoiCheck(String wonyoiCheck) {
		this.wonyoiCheck = wonyoiCheck;
	}

	public String getEmergencyCheck() {
		return emergencyCheck;
	}

	public void setEmergencyCheck(String emergencyCheck) {
		this.emergencyCheck = emergencyCheck;
	}

	public String getSpecimenCheck() {
		return specimenCheck;
	}

	public void setSpecimenCheck(String specimenCheck) {
		this.specimenCheck = specimenCheck;
	}

	public String getPortableYn2() {
		return portableYn2;
	}

	public void setPortableYn2(String portableYn2) {
		this.portableYn2 = portableYn2;
	}

	public String getBulyongCheck() {
		return bulyongCheck;
	}

	public void setBulyongCheck(String bulyongCheck) {
		this.bulyongCheck = bulyongCheck;
	}

	public String getSunabCheck() {
		return sunabCheck;
	}

	public void setSunabCheck(String sunabCheck) {
		this.sunabCheck = sunabCheck;
	}

	public String getDcCheck() {
		return dcCheck;
	}

	public void setDcCheck(String dcCheck) {
		this.dcCheck = dcCheck;
	}

	public String getDcGubunCheck() {
		return dcGubunCheck;
	}

	public void setDcGubunCheck(String dcGubunCheck) {
		this.dcGubunCheck = dcGubunCheck;
	}

	public String getConfirmCheck() {
		return confirmCheck;
	}

	public void setConfirmCheck(String confirmCheck) {
		this.confirmCheck = confirmCheck;
	}

	public String getReserYnCheck() {
		return reserYnCheck;
	}

	public void setReserYnCheck(String reserYnCheck) {
		this.reserYnCheck = reserYnCheck;
	}

	public String getChisikYn() {
		return chisikYn;
	}

	public void setChisikYn(String chisikYn) {
		this.chisikYn = chisikYn;
	}

	public String getNdayYn() {
		return ndayYn;
	}

	public void setNdayYn(String ndayYn) {
		this.ndayYn = ndayYn;
	}

	public String getDefltJaeryoJundal() {
		return defltJaeryoJundal;
	}

	public void setDefltJaeryoJundal(String defltJaeryoJundal) {
		this.defltJaeryoJundal = defltJaeryoJundal;
	}

	public String getDefltWonyoiYn() {
		return defltWonyoiYn;
	}

	public void setDefltWonyoiYn(String defltWonyoiYn) {
		this.defltWonyoiYn = defltWonyoiYn;
	}

	public String getSpecCmt() {
		return specCmt;
	}

	public void setSpecCmt(String specCmt) {
		this.specCmt = specCmt;
	}

	public String getSpecCmtName() {
		return specCmtName;
	}

	public void setSpecCmtName(String specCmtName) {
		this.specCmtName = specCmtName;
	}

	public String getSpecCmtSysId() {
		return specCmtSysId;
	}

	public void setSpecCmtSysId(String specCmtSysId) {
		this.specCmtSysId = specCmtSysId;
	}

	public String getSpecCmtPgmId() {
		return specCmtPgmId;
	}

	public void setSpecCmtPgmId(String specCmtPgmId) {
		this.specCmtPgmId = specCmtPgmId;
	}

	public String getSpecCmtNotNull() {
		return specCmtNotNull;
	}

	public void setSpecCmtNotNull(String specCmtNotNull) {
		this.specCmtNotNull = specCmtNotNull;
	}

	public String getSpecCmtTblId() {
		return specCmtTblId;
	}

	public void setSpecCmtTblId(String specCmtTblId) {
		this.specCmtTblId = specCmtTblId;
	}

	public String getSpecCmtColId() {
		return specCmtColId;
	}

	public void setSpecCmtColId(String specCmtColId) {
		this.specCmtColId = specCmtColId;
	}

	public String getDonbogYn() {
		return donbogYn;
	}

	public void setDonbogYn(String donbogYn) {
		this.donbogYn = donbogYn;
	}

	public String getOrdGubBasName() {
		return ordGubBasName;
	}

	public void setOrdGubBasName(String ordGubBasName) {
		this.ordGubBasName = ordGubBasName;
	}

	public String getActDoctor() {
		return actDoctor;
	}

	public void setActDoctor(String actDoctor) {
		this.actDoctor = actDoctor;
	}

	public String getActBuseo() {
		return actBuseo;
	}

	public void setActBuseo(String actBuseo) {
		this.actBuseo = actBuseo;
	}

	public String getActGwa() {
		return actGwa;
	}

	public void setActGwa(String actGwa) {
		this.actGwa = actGwa;
	}

	public String getHomeCareYn() {
		return homeCareYn;
	}

	public void setHomeCareYn(String homeCareYn) {
		this.homeCareYn = homeCareYn;
	}

	public String getRegularYn() {
		return regularYn;
	}

	public void setRegularYn(String regularYn) {
		this.regularYn = regularYn;
	}

	public Double getSortFkocskey() {
		return sortFkocskey;
	}

	public void setSortFkocskey(Double sortFkocskey) {
		this.sortFkocskey = sortFkocskey;
	}

	public String getChildYn() {
		return childYn;
	}

	public void setChildYn(String childYn) {
		this.childYn = childYn;
	}

	public String getIfInputCtrl() {
		return ifInputCtrl;
	}

	public void setIfInputCtrl(String ifInputCtrl) {
		this.ifInputCtrl = ifInputCtrl;
	}

	public String getChildGubun() {
		return childGubun;
	}

	public void setChildGubun(String childGubun) {
		this.childGubun = childGubun;
	}

	public String getSlipName() {
		return slipName;
	}

	public void setSlipName(String slipName) {
		this.slipName = slipName;
	}

	public String getNdayOccurYn() {
		return ndayOccurYn;
	}

	public void setNdayOccurYn(String ndayOccurYn) {
		this.ndayOccurYn = ndayOccurYn;
	}

	public Double getOrgKey() {
		return orgKey;
	}

	public void setOrgKey(Double orgKey) {
		this.orgKey = orgKey;
	}

	public Double getParentKey() {
		return parentKey;
	}

	public void setParentKey(Double parentKey) {
		this.parentKey = parentKey;
	}

	public String getBunCode() {
		return bunCode;
	}

	public void setBunCode(String bunCode) {
		this.bunCode = bunCode;
	}

	public String getWonnaeDrgYn() {
		return wonnaeDrgYn;
	}

	public void setWonnaeDrgYn(String wonnaeDrgYn) {
		this.wonnaeDrgYn = wonnaeDrgYn;
	}

	public String getHubalChangeCk() {
		return hubalChangeCk;
	}

	public void setHubalChangeCk(String hubalChangeCk) {
		this.hubalChangeCk = hubalChangeCk;
	}

	public String getDrgPackCheck() {
		return drgPackCheck;
	}

	public void setDrgPackCheck(String drgPackCheck) {
		this.drgPackCheck = drgPackCheck;
	}

	public String getPharmacyCheck() {
		return pharmacyCheck;
	}

	public void setPharmacyCheck(String pharmacyCheck) {
		this.pharmacyCheck = pharmacyCheck;
	}

	public String getPowderCheck() {
		return powderCheck;
	}

	public void setPowderCheck(String powderCheck) {
		this.powderCheck = powderCheck;
	}

	public String getOrderByKey() {
		return orderByKey;
	}

	public void setOrderByKey(String orderByKey) {
		this.orderByKey = orderByKey;
	}

}
