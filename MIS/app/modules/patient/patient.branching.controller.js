(function () {
    ...
    angular
        .module('app')
        .controller('PatientBranchingController', PatientBranchingController);

    PatientBranchingController.$inject = ['$scope', '$routeParams', 'cssInjector', '$location', 'SurveysService', 'DTOptionsBuilder', '$compile', 'DTColumnDefBuilder', 'gettextCatalog', '$http', '$timeout', 'localStorageService', '$cookieStore', '$interval', 'PatientService'];
    function PatientBranchingController($scope, $routeParams, cssInjector, $location, SurveysService, DTOptionsBuilder, $compile, DTColumnDefBuilder, gettextCatalog, $http, $timeout, localStorageService, $cookieStore, $interval, PatientService) {
        ...
        vm.user = null;
        vm.allUsers = [];
        var indexGroup, indexQuestionList, question = null;

        var listQuestionAnswered = [];
        var listQuestion ;
        $scope.listAnswered =[];
        var question_EndSurvey = [];
        ...
        function initController() {
            ...
            if ($location.path() == '/patient/view-survey/' + $routeParams.id) { // For view survey page
            ...
            } else if ($location.path() == '/patient/answers-survey-branching/' + $routeParams.patient_sid || $location.path() == '/patient/do-survey-only-branching/' + $routeParams.patient_sid) { // For do survey page
                    ...
                    if (res.status == 1) {
                        ...
                        if ($scope.surveyObj.survey_status == 1) {
                            ...
                            $.each($scope.surveyObj.survey_information.question_group, function (index, value) {
                                if (value.group_sequence == 0) {
                                    $.each(value.question_list, function (idx, val) {
                                        if (val.question_sequence == 0) {
                            ...
                            $scope.indexQuestionList = indexQuestionList;
                            $scope.listQuestionAnswered = listQuestionAnswered;
                        }
                    } else {
                        $location.path('/patient');
                    }
                ...
                if ($location.path() == '/patient') { // If does not passing anything
                    ...
                    var params = {
                        survey_status: $scope.selectedStatus,
                        reservation_from: searchObj.reservation_from,
                        reservation_to: searchObj.reservation_to
                    };
                    $scope.initDataTable(params);
                    ...
                    var searchObj = $scope.setDateTime($routeParams.type);
                    var params = {
                        department_code: $scope.selectedId,
                        reservation_from: searchObj.reservation_from,
                        reservation_to: searchObj.reservation_to
                    };
                    $scope.initDataTable(params);
        ...
        $scope.searchSurvey = function () {
            ...
            if ($('#select_patient').val() != '') {
                ...
                $scope.paramsSearch.patient = $('#select_patient').val();
            }
        ...
        $scope.nextQuestion = function () {
            ...
            var listAnswer = [];
            listQuestion = [];
            //Add question has been answered
            if (listQuestionAnswered.indexOf(question.question_id) == -1) {
                listQuestionAnswered.push(question.question_id);
            }
            //Check survey matrix >0
            if ($scope.surveyObj.survey_information.survey_matrix.length > 0) {
                $.each($scope.surveyObj.survey_information.survey_matrix, function (index_matrix, value_matrix) {
                    //check matrix has question which had been answer.
                    if (question.question_id == value_matrix.source_question_id) {
                        $.each(question.answer_list, function (index_answer, val_answer) {
                              if ((val_answer.is_selected == true && val_answer.answer_id == value_matrix.answer_id && question.question_id == value_matrix.source_question_id)
                                || ((question.has_other_text != undefined && question.has_other_text != '') && question.question_id == value_matrix.source_question_id && value_matrix.answer_id == -1)) {
                                // get answer terminate terminate
                                if (value_matrix.destination_question_id == 0) {
                                    if (question_EndSurvey.indexOf(question.question_id) == -1) {
                                        question_EndSurvey.push(question.question_id);
                                    }
                                }
                                else
                                {
                                    //get destination questions
                                    $.each($scope.surveyObj.survey_information.question_group, function (index_group, value_group) {
                                        $.each(value_group.question_list, function (index_question, value_question) {
                                            if (value_question.question_id == value_matrix.destination_question_id) {
                                                if ( value_question.question_sequence > question.question_sequence) {
                                                    listAnswer.push({
                                                        index: value_question.question_sequence,
                                                        value: value_question.question_id
                                                    });
                                ...
                                if(listAnswer.length > 0) {
                                    listAnswer.sort(function (a, b) {
                                        return a.index - b.index;
                                    })
                                    if( $scope.listAnswered.indexOf({index:question.question_id,value:listAnswer}) == -1)
                                    {
                                        $scope.listAnswered.push({index:question.question_id,value:listAnswer});
                                    }
                                    $.each($scope.listAnswered, function (index, value) {
                                        $.each(value.value, function (idx, val) {

                                            var found = listQuestion.some(function (el) {
                                                return el.value == val.value;
                                            });
                                            if (!found && val.value != value.index && val.value != question.question_id) {
                                                listQuestion.push(val);
                                            }
                                        })
                                    })
                                }
           ...
           /* listQuestion = listQuestion.filter(function(el) {
                return el.value !== b.value;
            });*/
            if (listQuestion.length > 0) {
                var listQuestionTemp = [];
                $.each(listQuestion, function (c, d) {
                    listQuestionTemp.push(d);
                })
                $.each(listQuestionAnswered, function (c, d) {
                    $.each(listQuestionTemp, function (a, b) {
                        if (b.value == d) {
                            listQuestion = listQuestion.filter(function (el) {
                                return el.value !== b.value && el.index > question.question_sequence;
                            });
                        }
                    })
                })
            ...
            if (listQuestion.length > 0) {
                ...
                $.each($scope.surveyObj.survey_information.question_group, function (index_group, value_group) {
                    $.each(value_group.question_list, function (index_question, value_question) {
                        ...
                        if (listQuestion[0] != undefined && value_question.question_id == listQuestion[0].value && !choice) {
                            if(value_question.question_type ==2)
                            {
                                value_question.input_text = '';
                            }
                            else
                            {
                                $.each(value_question.answer_list, function (i, v) {
                                    v.is_selected = false;
                                });
                            }
                            if(value_question.has_other_answer == 1)
                            {
                                value_question.has_other_text = '';
                            }
                            question = value_question;
                            indexQuestionList = index_question;
                ...
                // If has terminate so end survey
                if (question_EndSurvey.length > 0) {
                    return false;
                }
                if (question.question_sequence == $scope.surveyObj.survey_information.question_group[indexGroup].question_list.length - 1) {
                    if (indexGroup < $scope.surveyObj.survey_information.question_group.length - 1) {
                        indexGroup = indexGroup + 1;
                    }
                }
                var listQuestion = [];
                $.each($scope.surveyObj.survey_information.question_group[indexGroup].question_list, function (index, value) {
                    if (value.question_sequence > question.question_sequence) {
                        listQuestion.push({index: value.question_sequence, value: value.question_id});
                    }
                })
                ...
                if (listQuestion.length > 0) {
                    ...
                    var listQuestion_id = listQuestion[0].value;
                    $.each($scope.surveyObj.survey_information.question_group[indexGroup].question_list, function (index_question, value_question) {
                        if (value_question.question_id == listQuestion_id) {
                            ...
                            question = value_question;
                            listQuestion=listQuestion.filter(function (el) {
                                return el.value !== value_question.question_id;
                            });
                            indexQuestionList = index_question;
        ...
        $scope.previewQuestion = function () {
            ...
            if (listQuestionAnswered.length > 0) {
                var preview_question_id = null;
                preview_question_id = listQuestionAnswered[(listQuestionAnswered.length - 1)];

                var listAnsweredTemp = [];
                $.each($scope.listAnswered, function (c, d) {
                    listAnsweredTemp.push(d);
                })

                $.each($scope.surveyObj.survey_information.question_group, function (index, value) {
                    $.each(value.question_list, function (idx, val) {
                        if (val.question_id == preview_question_id) {
                            question = val;
                            indexGroup = index;
                            indexQuestionList = idx;
                            listQuestionAnswered.splice((listQuestionAnswered.length - 1), 1);

                            if (question_EndSurvey.indexOf(val.question_id) != -1) {

                                question_EndSurvey.splice(question_EndSurvey.indexOf(val.question_id), 1);
                            }

                            $scope.listAnswered =$scope.listAnswered.filter(function (el) {
                                  return el !== preview_question_id;
                            });
        ...
        $scope.finishedSurvey = function () {
            ...
            $.each($scope.question_group.group, function (index, value) {
                ...
                var listQuestion = [];
                $.each(value.question_list, function (u, t) {
                    if (listQuestionAnswered.indexOf(t.question_id) !== -1) {
                        listQuestion.push(t);
                    }
                })
                value.question_list = listQuestion;
                    ...
                    var dd = {
                        ...
                        footer: function (currentPage, pageCount) {
                            return {
                                stack: [
                                    {
                                        text: (currentPage == pageCount) ? gettextCatalog.getString('Thank you for your cooperation.') : '',
                                        alignment: 'right',
                                        fontSize: 10,
                                        margin: [0, 0, 30, 0]
                                    },
                                    ...
                                    {
                                        text: ($scope.hospitalObj.address == null) ? ' ' : $scope.hospitalObj.address,
                                        fontSize: 10,
                                        alignment: 'right',
                                        margin: [0, 0, 30, 0]
                                    }
                                ...
                                table: {
                                    ...
                                    body: [
                                        [{
                                            ...
                                            font: fontDefault
                                        }, ($scope.phrAccount.patien_id == null) ? '' : $scope.phrAccount.patien_id],
                                        [{
                                            text: gettextCatalog.getString('Patient name'),
                                            font: fontDefault
                                        }, ($scope.phrAccount.patient_name == null) ? '' : $scope.phrAccount.patient_name],
                                        [{
                                            text: gettextCatalog.getString('Answered date'),
                                            font: fontDefault
                                        }, ($scope.phrAccount.answer_date == null) ? '' : $scope.phrAccount.answer_date]
                                    ],
                                    font: fontDefault
                                },
                                margin: [65, 0, 0, 0]
                                ...
                                table: {
                                    ...
                                    body: [
                                        [{
                                            text: gettextCatalog.getString('Patient code'),
                                            font: fontDefault
                                        }, ($scope.surveyObjPdf.patient_code != null && $scope.surveyObjPdf.patient_code != '' ) ? $scope.surveyObjPdf.patient_code : ' '],
                                        [{
                                            text: gettextCatalog.getString('Patient name'),
                                            font: fontDefault
                                        }, ($scope.surveyObjPdf.patient_name != null && $scope.surveyObjPdf.patient_name != '') ? $scope.surveyObjPdf.patient_name : ' '],
                                        [{
                                            text: gettextCatalog.getString('Answered date'),
                                            font: fontDefault
                                        }, ($scope.surveyObjPdf.answer_date != null && $scope.surveyObjPdf.answer_date != '') ? $scope.surveyObjPdf.answer_date : ' ']
                                    ],
                                },
                                margin: [65, 0, 0, 0],
                                font: fontDefault
        ...
        $scope.quickSurvey = function () {
            ...
            if ($('#quickSurveyForm #department_code').val() != '') {
                if ($('#quickSurveyForm #department_code').val() != -1) {
                    $scope.params = {
                        survey_status: 0,
                        patient: $('#quickSurveyForm #patient_code').val(),
                        ...
                        if (res.status == 200) {
                            if (res.data.data.patient_survey_list.length > 0) {
                                ...
                                $.each(res.data.data.patient_survey_list, function (index, value) {
                                    if (value.patient_code == $('#quickSurveyForm #patient_code').val()) {
