    ...
    public partial class UcGrid : XtraUserControl
    {
        ...
        public void gridView1_MasterRowExpanded(object sender, CustomMasterRowEventArgs e)
        {
            ...
            if (gridViewWelds != null)
            {
                ...
                if (grdViewMd != null)
                {
                    ...
                    grdViewMd.Columns["OrderDisplayOrder"].Width = 30;
                    ...
                    {
                        grdViewMd.Columns["InputGubunName"].Width = 60;
                        grdViewMd.Columns["OrderGubunName"].Width = 65;
                        grdViewMd.Columns["HangmogName"].Width = 200;
                        grdViewMd.Columns["Content"].Width = 55;
                    }
        ...
        private void grdContent_ProcessGridKey(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Delete)
            {
                ...
                if (!currentRow["Type"].ToString().Equals("3")
                    && !currentRow["Type"].ToString().Equals("4") || gridView1.FocusedColumn.FieldName == "TagCode")
                {
                    gridView1.DeleteSelectedRows();
                    return;
                }
        ...
        private List<OrderInfo> GetCurrentOrders()
        {
            ...
            if (lstOrder != null)
            {
                ...
                foreach (DataRow row1 in dRows)
                {
                    ...
                    orderItem.Content = row1["suryang"] + orderItem.OrdDanuiName + row1["dv_time"] + row1["dv"];
            ...
            foreach (EmrRecordInfo emrRecordInfo in lstRecord)
            {
                ...
                foreach (TagInfo tagInfo in emrRecordInfo.TagInfos)
                {
                    ...
                    else if (rpName.Trim().ToUpper() == B2)
                    {
                        ...
                        #endregion
                        //if (tagInfo.Content != null && !String.IsNullOrEmpty(tagInfo.Content.ToString()))
                        //{
                        //    tagInfo.TagCode = emrRecordInfo.TagInfos[i].TagCode;
                        //    tagInfo.TagName = emrRecordInfo.TagInfos[i].TagName;
                        //    tagInfo.Content = emrRecordInfo.TagInfos[i].Content;
                        //    lstTagInfos.Add(tagInfo);
                        //}
