...
@implementation PHRLifeStyleChart{
    float maxLineYAxis;
    float average;
    NSString *chartTitle;
...
- (void)calculateMinMax {
    ...
    if (self.arrayAllData.count > 0) {
        for (LifeStyleNoteModel *obj in self.arrayAllData){
            double hour = round (10 * ([obj.total_hour_sleep floatValue]/3600)) / 10;
            if (self.maxAxisValue < hour) {
                self.maxAxisValue = hour;
                maxLineYAxis= self.maxAxisValue;
            }
            if (self.minAxisValue > hour) {
                self.minAxisValue = hour;
            }
            average += hour;
        }
...
- (void)processData {
    ...
    if (self.arrayAllData.count > 0) {
        for (LifeStyleNoteModel *obj in self.arrayAllData) {
            dateTime = [UIUtils dateFromServerTimeString:obj.time_start_sleep];
            [self.arrayXData addObject:[UIUtils stringDate:dateTime withFormat:@"MM/dd"]];
        }
        
        for (int i = 0; i < self.arrayAllData.count ; i++) {
            LifeStyleNoteModel *obj = (LifeStyleNoteModel*) self.arrayAllData[i];
            ChartDataEntry *chartData = [[ChartDataEntry alloc] initWithValue:round (10 * ([obj.total_hour_sleep floatValue]/3600)) / 10 xIndex:i];
...
- (void)addLimitLine {
    ...
    [self addLimitLineOnRightAxis:maxLineYAxis * 1.1f lineWidth:self.limitLineWidth * 2.0 lineColor:[UIColor whiteColor] textColor:[UIColor whiteColor] andText:[NSString stringWithFormat:@"%@ %0.1f %@",kLocalizedString(kTitleAverage),average, kLocalizedString(kHour)] withFont:[FontUtils aileronRegularWithSize:8.0] andTextDirection:ChartLimitLabelPositionRightTop];
