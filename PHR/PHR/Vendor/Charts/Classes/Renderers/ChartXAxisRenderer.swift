...
public class ChartXAxisRenderer: ChartAxisRendererBase
{
    ...
    public func drawLabels(context context: CGContext, pos: CGFloat, anchor: CGPoint)
    {
        ...
        if (xAxis.values.count == 1){
            let label = xAxis.values[0]
            if (label == nil)
            {
                return;
            }
            ...
            if (viewPortHandler.isInBoundsX(position.x))
            {
                ...
                drawLabel(context: context, label: label!, xIndex: 0, x: position.x, y: pos, attributes: labelAttrs, constrainedToSize: labelMaxSize, anchor: anchor, angleRadians: labelRotationAngleRadians)
            }
