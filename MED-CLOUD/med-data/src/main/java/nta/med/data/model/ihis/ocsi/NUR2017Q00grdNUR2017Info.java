package nta.med.data.model.ihis.ocsi;

import java.math.BigDecimal;
import java.math.BigInteger;
import java.util.Date;

public class NUR2017Q00grdNUR2017Info {
    private String hoCode1 ;
    private String bunho ;
    private String suname ;
    private String gwaName ;
    private String hangmogCode ;
    private String hangmogName ;
    private String suryang ;
    private String ordDanui ;
    private String dvTime ;
    private BigInteger dv ;
    private String jusa ;
    private String bogyongCode ;
    private Date actingDate ;
    private String actingTime ;
    private String actUser ;
    private String actingYn ;
    private Double fkocs2003 ;
    private BigDecimal seq ;
    private Date orderDate ;
    private String bogyongName ;
    private String dcYn ;
    private Double groupSer ;
    private String mixGroup ;
    private Date hopeDate ;
    private String jusaText ;
    private String danuiText ;
    private String orderRemark ;
    private String nurseRemark ;
    private String inputGubunText ;
    private String jusaSpdGubun ;
    private String actUserName ;
    private String bannabYn ;
    private Double bannabFkocs2003 ;
    private String oldBannabYn ;
	public NUR2017Q00grdNUR2017Info(String hoCode1, String bunho, String suname, String gwaName, String hangmogCode,
			String hangmogName, String suryang, String ordDanui, String dvTime, BigInteger dv, String jusa,
			String bogyongCode, Date actingDate, String actingTime, String actUser, String actingYn, Double fkocs2003,
			BigDecimal seq, Date orderDate, String bogyongName, String dcYn, Double groupSer, String mixGroup,
			Date hopeDate, String jusaText, String danuiText, String orderRemark, String nurseRemark,
			String inputGubunText, String jusaSpdGubun, String actUserName, String bannabYn, Double bannabFkocs2003,
			String oldBannabYn) {
		super();
		this.hoCode1 = hoCode1;
		this.bunho = bunho;
		this.suname = suname;
		this.gwaName = gwaName;
		this.hangmogCode = hangmogCode;
		this.hangmogName = hangmogName;
		this.suryang = suryang;
		this.ordDanui = ordDanui;
		this.dvTime = dvTime;
		this.dv = dv;
		this.jusa = jusa;
		this.bogyongCode = bogyongCode;
		this.actingDate = actingDate;
		this.actingTime = actingTime;
		this.actUser = actUser;
		this.actingYn = actingYn;
		this.fkocs2003 = fkocs2003;
		this.seq = seq;
		this.orderDate = orderDate;
		this.bogyongName = bogyongName;
		this.dcYn = dcYn;
		this.groupSer = groupSer;
		this.mixGroup = mixGroup;
		this.hopeDate = hopeDate;
		this.jusaText = jusaText;
		this.danuiText = danuiText;
		this.orderRemark = orderRemark;
		this.nurseRemark = nurseRemark;
		this.inputGubunText = inputGubunText;
		this.jusaSpdGubun = jusaSpdGubun;
		this.actUserName = actUserName;
		this.bannabYn = bannabYn;
		this.bannabFkocs2003 = bannabFkocs2003;
		this.oldBannabYn = oldBannabYn;
	}
	public String getHoCode1() {
		return hoCode1;
	}
	public void setHoCode1(String hoCode1) {
		this.hoCode1 = hoCode1;
	}
	public String getBunho() {
		return bunho;
	}
	public void setBunho(String bunho) {
		this.bunho = bunho;
	}
	public String getSuname() {
		return suname;
	}
	public void setSuname(String suname) {
		this.suname = suname;
	}
	public String getGwaName() {
		return gwaName;
	}
	public void setGwaName(String gwaName) {
		this.gwaName = gwaName;
	}
	public String getHangmogCode() {
		return hangmogCode;
	}
	public void setHangmogCode(String hangmogCode) {
		this.hangmogCode = hangmogCode;
	}
	public String getHangmogName() {
		return hangmogName;
	}
	public void setHangmogName(String hangmogName) {
		this.hangmogName = hangmogName;
	}
	public String getSuryang() {
		return suryang;
	}
	public void setSuryang(String suryang) {
		this.suryang = suryang;
	}
	public String getOrdDanui() {
		return ordDanui;
	}
	public void setOrdDanui(String ordDanui) {
		this.ordDanui = ordDanui;
	}
	public String getDvTime() {
		return dvTime;
	}
	public void setDvTime(String dvTime) {
		this.dvTime = dvTime;
	}
	public BigInteger getDv() {
		return dv;
	}
	public void setDv(BigInteger dv) {
		this.dv = dv;
	}
	public String getJusa() {
		return jusa;
	}
	public void setJusa(String jusa) {
		this.jusa = jusa;
	}
	public String getBogyongCode() {
		return bogyongCode;
	}
	public void setBogyongCode(String bogyongCode) {
		this.bogyongCode = bogyongCode;
	}
	public Date getActingDate() {
		return actingDate;
	}
	public void setActingDate(Date actingDate) {
		this.actingDate = actingDate;
	}
	public String getActingTime() {
		return actingTime;
	}
	public void setActingTime(String actingTime) {
		this.actingTime = actingTime;
	}
	public String getActUser() {
		return actUser;
	}
	public void setActUser(String actUser) {
		this.actUser = actUser;
	}
	public String getActingYn() {
		return actingYn;
	}
	public void setActingYn(String actingYn) {
		this.actingYn = actingYn;
	}
	public Double getFkocs2003() {
		return fkocs2003;
	}
	public void setFkocs2003(Double fkocs2003) {
		this.fkocs2003 = fkocs2003;
	}
	public BigDecimal getSeq() {
		return seq;
	}
	public void setSeq(BigDecimal seq) {
		this.seq = seq;
	}
	public Date getOrderDate() {
		return orderDate;
	}
	public void setOrderDate(Date orderDate) {
		this.orderDate = orderDate;
	}
	public String getBogyongName() {
		return bogyongName;
	}
	public void setBogyongName(String bogyongName) {
		this.bogyongName = bogyongName;
	}
	public String getDcYn() {
		return dcYn;
	}
	public void setDcYn(String dcYn) {
		this.dcYn = dcYn;
	}
	public Double getGroupSer() {
		return groupSer;
	}
	public void setGroupSer(Double groupSer) {
		this.groupSer = groupSer;
	}
	public String getMixGroup() {
		return mixGroup;
	}
	public void setMixGroup(String mixGroup) {
		this.mixGroup = mixGroup;
	}
	public Date getHopeDate() {
		return hopeDate;
	}
	public void setHopeDate(Date hopeDate) {
		this.hopeDate = hopeDate;
	}
	public String getJusaText() {
		return jusaText;
	}
	public void setJusaText(String jusaText) {
		this.jusaText = jusaText;
	}
	public String getDanuiText() {
		return danuiText;
	}
	public void setDanuiText(String danuiText) {
		this.danuiText = danuiText;
	}
	public String getOrderRemark() {
		return orderRemark;
	}
	public void setOrderRemark(String orderRemark) {
		this.orderRemark = orderRemark;
	}
	public String getNurseRemark() {
		return nurseRemark;
	}
	public void setNurseRemark(String nurseRemark) {
		this.nurseRemark = nurseRemark;
	}
	public String getInputGubunText() {
		return inputGubunText;
	}
	public void setInputGubunText(String inputGubunText) {
		this.inputGubunText = inputGubunText;
	}
	public String getJusaSpdGubun() {
		return jusaSpdGubun;
	}
	public void setJusaSpdGubun(String jusaSpdGubun) {
		this.jusaSpdGubun = jusaSpdGubun;
	}
	public String getActUserName() {
		return actUserName;
	}
	public void setActUserName(String actUserName) {
		this.actUserName = actUserName;
	}
	public String getBannabYn() {
		return bannabYn;
	}
	public void setBannabYn(String bannabYn) {
		this.bannabYn = bannabYn;
	}
	public Double getBannabFkocs2003() {
		return bannabFkocs2003;
	}
	public void setBannabFkocs2003(Double bannabFkocs2003) {
		this.bannabFkocs2003 = bannabFkocs2003;
	}
	public String getOldBannabYn() {
		return oldBannabYn;
	}
	public void setOldBannabYn(String oldBannabYn) {
		this.oldBannabYn = oldBannabYn;
	}
	
}
