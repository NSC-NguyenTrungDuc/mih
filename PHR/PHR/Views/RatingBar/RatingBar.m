...
- (id)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        for(int i = 0;i<5;i++){
            ...
            img.center = CGPointMake((i+1.5)*width, frame.size.height/1.5f);
            ...
            img2.image = [UIImage imageNamed:@"icon_star"];
...
-(void)setStarNumber:(NSInteger)starNumber{
    ...
    if(self.delegate){
        [self.delegate setRating:_starNumber isHuman:NO];
    }
}
...
-(void)tap:(UITapGestureRecognizer *)gesture{
    ...
    if(self.delegate){
        [self.delegate setRating:_starNumber isHuman:YES];
    }
}
