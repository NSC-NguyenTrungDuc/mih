...
@Service                                                                                                          
@Scope("prototype")                                                                                 
public class OCSACTLayconstantConstHandler extends ScreenHandler<OcsoServiceProto.OCSACTLayconstantConstRequest, SystemServiceProto.LayConstantInfoResponse> {                     
	private static final Log LOGGER = LogFactory.getLog(OCSACTLayconstantConstHandler.class);                                    
	@Resource                                                                                                       
	...
	public SystemServiceProto.LayConstantInfoResponse handle(Vertx vertx, String clientId,
			...
			OcsoServiceProto.OCSACTLayconstantConstRequest request) throws Exception {
		SystemServiceProto.LayConstantInfoResponse.Builder response = SystemServiceProto.LayConstantInfoResponse.newBuilder(); 
		List<LayConstantInfo> listConstant = pfe0102Repository.getOCSACTLayconstant(getHospitalCode(vertx, sessionId), "PFE_CONSTANT", getLanguage(vertx, sessionId));
