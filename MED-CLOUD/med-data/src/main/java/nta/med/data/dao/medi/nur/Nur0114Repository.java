 ...
 */
@Repository
public interface Nur0114Repository extends JpaRepository<Nur0114, Long>, Nur0114RepositoryCustom {

	@Query("SELECT T FROM Nur0114 T WHERE T.hospCode = :hospCode AND T.nurGrCode = :nurGrCode AND T.nurMdCode = :nurMdCode AND T.nurSoCode = :nurSoCode")
	public List<Nur0114> findByHospCodeNurGrCodeNurMdCodeNurSoCode( @Param("hospCode") String hospCode, 
																	@Param("nurGrCode") String nurGrCode, 
																	@Param("nurMdCode") String nurMdCode, 
																	@Param("nurSoCode") String nurSoCode);
	
	@Modifying
	@Query(   " UPDATE Nur0114                           "
			+ " SET updId        = :q_user_id,           "
			+ "    updDate       = SYSDATE(),            "
	...
	public Integer updateByHospCodeNurGrCodeNurMdCodeNurSoCode(
	...
	@Query(   " UPDATE Nur0114                           "
			...
			+ "   AND nurMdCode  = :f_nur_md_code        "
			+ "   AND nurSoCode  = :f_nur_so_code        ")
	public Integer updateValdByHospCodeNurGrCodeNurMdCodeNurSoCode(
			@Param("q_user_id") String userId ,
			@Param("f_vald") String vald,
			@Param("f_hosp_code") String hospCode, 
			@Param("f_nur_gr_code") String nurGrCode, 
