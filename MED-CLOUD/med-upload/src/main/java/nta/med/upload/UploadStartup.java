...
public class UploadStartup {
    ...
    private static final Log LOGGER = LogFactory.getLog(UploadStartup.class);
    ...
    public static void main(String[] args) {
        ...
        URL[] classpath = {};
        pm.deployVerticle("nta.med.upload.UploadVerticle", config, classpath, instances, null, new Handler<AsyncResult<String>>() {
            @Override
            public void handle(AsyncResult<String> stringAsyncResult) {
                if (stringAsyncResult.succeeded()) {
                    LOGGER.info("Completed deployment of UploadVerticle");
                    HazelcastManager hm = new HazelcastManager();
                    hm.start();
                } else {
                    LOGGER.info(String.format("Failed deployment of UploadVerticle with result ", stringAsyncResult.result()));
                }
            }
        });
    }
