...
import nta.med.service.emr.proto.EmrModelProto.OCS2015U09EmrBookmarkInfo;
...
@Scope("prototype")
@Transactional
public class OCS2015U09EmrRecordUpdateHandler extends ScreenHandler<EmrServiceProto.OCS2015U09EmrRecordUpdateRequest, SystemServiceProto.UpdateResponse> {
	private static final Log LOGGER = LogFactory.getLog(OCS2015U09EmrRecordUpdateHandler.class);                                    
	@Resource
	...
	@Override                                                                                                       
	public SystemServiceProto.UpdateResponse handle(Vertx vertx, String clientId, String sessionId, long contextId, EmrServiceProto.OCS2015U09EmrRecordUpdateRequest request) throws Exception {
		String hospCode = getHospitalCode(vertx, sessionId);
		LOG.info("OCS2015U09EmrRecordUpdateHandler EMR sessionId "+ sessionId);
		LOG.info("OCS2015U09EmrRecordUpdateHandler EMR hospCode" + hospCode);
	...
	public Integer finishExaminationProgess(EmrServiceProto.OCS2015U09EmrRecordUpdateRequest request, String hospitalCode, String language) {
		...
		if (medicalRecord != null && medicalRecord.getEmrRecordId() != null) {//Medical record is exist
			...
			if (request.getSysId().equals(updId)) {
				...
				LOG.info("call OCS2015U09EmrRecordUpdateHandler setLockForUser");
				...
				LOG.info("call OCS2015U09EmrRecordUpdateHandler getEMRUserGetLockEditingInfo");
				if (emrUser.getLockFlg().compareTo(new BigDecimal(0))==0) { //Lock flag = 0
					//[4] Update editing lock
					LOG.info("call OCS2015U09EmrRecordUpdateHandler setLockForUser 1");
					result = emrRecordRepository.setLockForUser(updId, medicalRecord.getEmrRecordId());
					...
					if(emrRecordRepository.getEditingTime (medicalRecord.getEmrRecordId()+"")==false) {
						//[4] Update editing lock
						LOG.info("call OCS2015U09EmrRecordUpdateHandler setLockForUser 2");
						result = emrRecordRepository.setLockForUser(updId, medicalRecord.getEmrRecordId());
					} else {
			...
			if(limitVesrion != null){
				...
				LOG.info("OCS2015U09EmrRecordUpdateHandler EMR limitVesrion isNull");
			}
			LOG.info("call OCS2015U09EmrRecordUpdateHandler updateEmrRepositoryHistory");
			//[7] Update new content of medical record
			save = updateEmrRepositoryHistory(medicalRecord.getEmrRecordId().toString(), request.getMetadata(), request.getContent(),
					lastVersion+2, request.getSysId(), request.getRecordLog());
			if(!save){
				return null;
			}
			...
			LOG.info("call insertOCS2015U07EmrRecord:");
			emrRecordId = insertOCS2015U07EmrRecord(request, hospitalCode);
			if (emrRecordId==null) {
				return null;
			}
		}
	...
	private Integer insertOCS2015U07EmrRecord (EmrServiceProto.OCS2015U09EmrRecordUpdateRequest request, String hospitalCode){
		try{
			LOG.info("OCS2015U09EmrRecordUpdateHandler EMR insertOCS2015U07EmrRecord " + hospitalCode);
			EmrRecord emrRecord = new EmrRecord();
			emrRecord.setActiveFlg(new BigDecimal(1));
			LOG.info("OCS2015U09EmrRecordUpdateHandler EMR setBunho " + request.getBunho());
			emrRecord.setBunho(request.getBunho());
			LOG.info("OCS2015U09EmrRecordUpdateHandler EMR setContent " + request.getContent());
			...
			LOG.info("OCS2015U09EmrRecordUpdateHandler EMR hospCode setter " + hospitalCode);
	...
	private boolean insertOCS2015U04AddBookmark (EmrServiceProto.OCS2015U09EmrRecordUpdateRequest request, Integer emrRecordId) {
		List<OCS2015U09EmrBookmarkInfo> listBookMark = request.getBookmarkList();
		try {
