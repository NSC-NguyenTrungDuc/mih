...
#define IMAGE_PLUS          @"Icon_Setting_Plus"
#define IMAGE_REMOVE        @"Icon_Setting_Remove"

#pragma mark - ButtonView

@interface ButtonView () {
    UIImageView *imgPlusOrRemove;
}
...
- (void)setupToButtonInView {
    ...
    [self.textLabel setNumberOfLines:2.0];
    
    self.textLabel.text = self.text;
    self.textLabel.backgroundColor = [UIColor clearColor];
    self.textLabel.font = [FontUtils aileronRegularWithSize:11.0];
    self.textLabel.textAlignment = NSTextAlignmentCenter;
    
    self.imageButton = [UIButton buttonWithType:UIButtonTypeCustom];
    
    [self setupToImageButtoninView];
    ...
    imgPlusOrRemove = [[UIImageView alloc] initWithFrame:CGRectMake(35.0, 28.0, 20.0, 20.0)];
    if (!self.isSelected) {
        imgPlusOrRemove.image = [UIImage imageNamed:IMAGE_REMOVE];
    }
    else {
        imgPlusOrRemove.image = [UIImage imageNamed:IMAGE_PLUS];
    }
    [self.imageButton addSubview:imgPlusOrRemove];
    ...
    //label紧贴view的左右
    constraints = [NSLayoutConstraint constraintsWithVisualFormat:@"H:|-(-6)-[textLabel]|" options:0 metrics:nil views:views];
    [self addConstraints:constraints];
    
    //imageView距离view左右各10, imageView的宽为50
    constraints = [NSLayoutConstraint constraintsWithVisualFormat:@"H:|-12-[imageButton(50)]-0-|" options:0 metrics:nil views:views];
    [self addConstraints:constraints];
    
    //竖直方向imageView和textLabel在一条直线上, 并且挨着, imageView的高为50
    constraints = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|[imageButton(50)][textLabel(36)]|" options:NSLayoutFormatAlignAllCenterX metrics:nil views:views];
    [self addConstraints:constraints];
}

- (void)setupToImageButtoninView {
    if (self.isSelected) {
        [self.textLabel setTextColor:PHR_COLOR_DATE_TIME];
        imgPlusOrRemove.image = [UIImage imageNamed:IMAGE_PLUS];
        [self.imageButton setImage:self.imageSelected forState:UIControlStateNormal];
    }
    else {
        [self.textLabel setTextColor:PHR_COLOR_LINE_GRAY];
        imgPlusOrRemove.image = [UIImage imageNamed:IMAGE_REMOVE];
...
- (void)buttonClicked:(UIButton *)button {
    if (self.handler) {
        NSLog(@"Before: self.isSelected: %d",self.isSelected);
        self.isSelected = !self.isSelected;
        NSLog(@"After: self.isSelected: %d",self.isSelected);
        [self setupToImageButtoninView];
        self.handler(self);
    }
    
    if (![self.textLabel.text isEqualToString:@""]) {
        StandardItemSelected *objStandard = [[StandardItemSelected alloc] init];
        objStandard.isSelected = self.isSelected;
        objStandard.type = self.type;
        
        [[NSNotificationCenter defaultCenter] postNotificationName:SelectedPropertyInStandard object:objStandard];
    }
}
...
- (void)setup {
    ...
    [self.cancelButton.layer setBorderColor:PHR_COLOR_GRAY.CGColor];
    [self.cancelButton.layer setBorderWidth:0.5 ];
    
    [self.cancelButton setTitle:kLocalizedString(kSave) forState:UIControlStateNormal];
    [self.cancelButton setTitleColor:PHR_COLOR_GRAY forState:UIControlStateNormal];
    [self.cancelButton setBackgroundColor:PHR_COLOR_LINE_GRAY];
    
    [self.cancelButton.titleLabel setStyleRegularToLabelWithSize:14.0];
    [self.cancelButton addTarget:self action:@selector(saveButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)updateConstraints {
    ...
    constraints = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|-5-[titleLabel(==0)]-[iconView]-[cancelButton(==45)]-0-|" options:0 metrics:nil views:views];
    [self.contentView addConstraints:constraints];
}
