...
import nta.med.service.ihis.proto.OcsiServiceProto.OCS6010U10PopupMASavePerformerRequest;
import nta.med.service.ihis.proto.SystemServiceProto;
import nta.med.service.ihis.proto.SystemServiceProto.UpdateResponse;

@Service
@Scope("prototype")
public class OCS6010U10PopupMASavePerformerHandler extends
		ScreenHandler<OcsiServiceProto.OCS6010U10PopupMASavePerformerRequest, SystemServiceProto.UpdateResponse> {

	private static final Log LOGGER = LogFactory.getLog(OCS6010U10PopupMASavePerformerHandler.class);
	...
	public UpdateResponse handle(Vertx vertx, String clientId, String sessionId, long contextId,
			OCS6010U10PopupMASavePerformerRequest request) throws Exception {
		...
		List<OcsiModelProto.OCS6010U10PopupMAgrdOCS2016Info> grdocs2016 = request.getGrdocs2016List();
		...
		for (OcsiModelProto.OCS6010U10PopupO2AgrdOCS2015Info info : grdocs2015) {
			if(DataRowState.DELETED.getValue().equals(info.getDataRowState())){
				if (!StringUtils.isEmpty(info.getActDate()) && !StringUtils.isEmpty(info.getStartTime())
					...
					CommonProcResultInfo pResult = ocs2015Repository.callPrOcsiIsJisiDateime(hospCode, "D", "MN"
							, info.getBunho()
					...
					if(!"T".equals(pResult.getStrResult1())){
						LOGGER.info(String.format("Execute PR_OCSI_IS_JISI_DATETIME <D/MN> fail, hosp_code = %s, Err = %s", hospCode, pResult.getStrResult1()));
	...
	private UpdateResponse.Builder handleGrdocs2015(String hospCode, String userId, Double fkocs2005, String directCode,
			...
			List<OcsiModelProto.OCS6010U10PopupMAgrdOCS2016Info> grdocs2016) {
		SystemServiceProto.UpdateResponse.Builder response = SystemServiceProto.UpdateResponse.newBuilder();
			...
			if(DataRowState.ADDED.getValue().equals(info.getDataRowState())){
				for (int i = 1; i <= processDays; i++) {
					...
					if(actDate != null && !StringUtils.isEmpty(startTime) && endDate != null && !StringUtils.isEmpty(endTime)){
						CommonProcResultInfo pResult = ocs2015Repository.callPrOcsiIsJisiDateime(hospCode, "I", "MN"
								, info.getBunho()
								, CommonUtils.parseDouble(info.getFkinp1001())
								, actDate
								, startTime
						...
						if(!"T".equals(pResult.getStrResult1())){
							LOGGER.info(String.format("Execute PR_OCSI_IS_JISI_DATETIME <I/MN> fail, hosp_code = %s, Err = %s", hospCode, pResult.getStrResult1()));
							response.setResult(false);
							return response;
						}
					}
					...
					// Handle Grid OCS2016
					boolean saveOcs2016Result = handleGrdOcs2016CaseAddedOcs2015(hospCode, userId, pkocs2015, grdocs2016, i);
					...
					// Execute procedure PR_OCS_DIRECT_ACT_MONITER
					CommonProcResultInfo pResult = ocs6010Repository.callPrOcsDirectActMoniter(hospCode
							, info.getBunho()
							, CommonUtils.parseDouble(info.getFkinp1001())
					...
					if(!"0".equals(pResult.getStrResult2())){
						LOGGER.info(String.format("Execute PR_OCS_DIRECT_ACT_MONITER <CASE ADDED> fail: hosp_code = %s, IO_MSG = %s, IO_FLAG = %s"
				...
				for (int i = 1; i <= processDays; i++) {
					...
					if(actDate != null && !StringUtils.isEmpty(startTime) && endDate != null && !StringUtils.isEmpty(endTime)){
						...
						if(!"T".equals(pResult.getStrResult1())){
							LOGGER.info(String.format("Execute PR_OCSI_IS_JISI_DATETIME <I/MN MODIFIED> fail, hosp_code = %s, Err = %s", hospCode, pResult.getStrResult1()));
							response.setResult(false);
							return response;
						}
					}
	...
	private boolean handleGrdOcs2016CaseAddedOcs2015(String hospCode, String userId, Double pkocs2015, List<OcsiModelProto.OCS6010U10PopupMAgrdOCS2016Info> lstOcs2016, int i){
		for (OcsiModelProto.OCS6010U10PopupMAgrdOCS2016Info info : lstOcs2016) {
			...
			if(i != 1){
				...
				CommonUtils.parseDouble(strSeq);
			}
	...
	private boolean handleGrdOcs2016CaseModifiedOcs2015(String hospCode, String userId, Double pkocs2015, List<OcsiModelProto.OCS6010U10PopupMAgrdOCS2016Info> grdocs2016, int i){
		for (OcsiModelProto.OCS6010U10PopupMAgrdOCS2016Info info : grdocs2016) {
			Double seq = CommonUtils.parseDouble(info.getSeq());
