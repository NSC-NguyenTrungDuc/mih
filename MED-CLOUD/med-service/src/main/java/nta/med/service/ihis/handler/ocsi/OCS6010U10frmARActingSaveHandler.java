...
import nta.med.service.ihis.proto.OcsiServiceProto.OCS6010U10frmARActingSaveRequest;
import nta.med.service.ihis.proto.SystemServiceProto;
import nta.med.service.ihis.proto.SystemServiceProto.UpdateResponse;

@Service
@Scope("prototype")
public class OCS6010U10frmARActingSaveHandler
		extends ScreenHandler<OcsiServiceProto.OCS6010U10frmARActingSaveRequest, SystemServiceProto.UpdateResponse> {

	private static final Log LOGGER = LogFactory.getLog(OCS6010U10frmARActingSaveHandler.class);
	...
	public UpdateResponse handle(Vertx vertx, String clientId, String sessionId, long contextId,
			OCS6010U10frmARActingSaveRequest request) throws Exception {
		...
		List<OcsiModelProto.OCS6010U10frmARActinggrdOCS2015Info> lstOcs2015 = request.getItems2015List();
		List<OcsiModelProto.OCS6010U10frmARActinggrdOCS2016Info> lstOcs2016 = request.getItems2016List();
		
		// grdOCS2015.DeletedRowCount
		List<OcsiModelProto.OCS6010U10frmARActinggrdOCS2015Info> lstOcs2015Deleted = new ArrayList<>();
		for (OcsiModelProto.OCS6010U10frmARActinggrdOCS2015Info info : lstOcs2015) {
			if(DataRowState.DELETED.getValue().equals(info.getDataRowState())){
				lstOcs2015Deleted.add(info);
			}
		}
		
		if(!CollectionUtils.isEmpty(lstOcs2015Deleted)){
			for (OcsiModelProto.OCS6010U10frmARActinggrdOCS2015Info info : lstOcs2015Deleted) {
				response = handleOcs2015Deleted(hospCode, info);
				if(!response.getResult()){
					LOGGER.info(String.format("Handle DeletedRow fail, hosp_code = %s", hospCode));
					return response.build();
				}
			}
		}
		
		response = handleGrdocs2015(hospCode, userId, fkocs2005, lstOcs2015, lstOcs2016);
		if(!response.getResult()){
	...
	public UpdateResponse.Builder handleOcs2015Deleted(String hospCode, OcsiModelProto.OCS6010U10frmARActinggrdOCS2015Info info){
		...
		if (!StringUtils.isEmpty(info.getActDate()) && !StringUtils.isEmpty(info.getStartTime())
			...
			CommonProcResultInfo pResult = ocs2015Repository.callPrOcsiIsJisiDateime(hospCode, "D", "AR"
																, info.getBunho()
			...
			if(!"T".equals(pResult.getStrResult1())){
				LOGGER.info(String.format("Execute PR_OCSI_IS_JISI_DATETIME <D> fail, hosp_code = %s, Err = %s", hospCode, pResult.getStrResult1()));
				response.setResult(false);
				return response;
			}
			
			// Execute PR_OCSI_MARUME_IUD
	...
	private UpdateResponse.Builder handleGrdocs2015(String hospCode, String userId, Double fkocs2005,
			List<OcsiModelProto.OCS6010U10frmARActinggrdOCS2015Info> grdocs2015,
			List<OcsiModelProto.OCS6010U10frmARActinggrdOCS2016Info> grdocs2016) {
		
		UpdateResponse.Builder response = UpdateResponse.newBuilder();
		
		for (OcsiModelProto.OCS6010U10frmARActinggrdOCS2015Info info : grdocs2015) {
			...
			if(DataRowState.ADDED.getValue().equals(info.getDataRowState())){
				for (int i = 1; i <= processDays; i++) {
					...
					if(actDate != null && !StringUtils.isEmpty(startTime) && endDate != null && !StringUtils.isEmpty(endTime)){
						CommonProcResultInfo pResult = ocs2015Repository.callPrOcsiIsJisiDateime(hospCode, "I", "AR"
								, info.getBunho()
								, CommonUtils.parseDouble(info.getFkinp1001())
								, actDate
								, startTime
						...
						if(!"T".equals(pResult.getStrResult1())){
							LOGGER.info(String.format("Execute PR_OCSI_IS_JISI_DATETIME <I/AR> fail, hosp_code = %s, Err = %s", hospCode, pResult.getStrResult1()));
							response.setResult(false);
							return response;
						}
					}
					...
					// Handle Grid OCS2016
					boolean saveOcs2016Result = handleGrdOcs2016CaseAddedOcs2015(hospCode, userId, pkocs2015, grdocs2016, info.getSuryang());
					...
					// Execute procedure PR_OCS_DIRECT_ACT_AR
					CommonProcResultInfo pResult = ocs6010Repository.callPrOcsDirectActAr(hospCode
							, info.getBunho()
							, CommonUtils.parseDouble(info.getFkinp1001())
					...
					if(!"0".equals(pResult.getStrResult2())){
						LOGGER.info(String.format("Execute PR_OCS_DIRECT_ACT_AR <CASE ADDED> fail: hosp_code = %s, IO_MSG = %s, IO_FLAG = %s"
				...
				for (int i = 1; i <= processDays; i++) {
					...
					if(actDate != null && !StringUtils.isEmpty(startTime) && endDate != null && !StringUtils.isEmpty(endTime)){
						...
						if(!"T".equals(pResult.getStrResult1())){
							LOGGER.info(String.format("Execute PR_OCSI_IS_JISI_DATETIME <I/AR MODIFIED> fail, hosp_code = %s, Err = %s", hospCode, pResult.getStrResult1()));
							response.setResult(false);
							return response;
						}
					}
					...
					if(i == 1){
						...
						ocs2015Repository.updateOcs2015InOCS6010U10frmARActingSave(hospCode
								...
								, CommonUtils.parseDouble(info.getChangeQty())
								, CommonUtils.parseDouble(info.getFio2())
								, startTime
								, endTime
								, endDate);
						...
						if(!"0".equals(pResult.getStrResult2())){
							LOGGER.info(String.format("Execute PR_OCS_DIRECT_ACT_AR <CASE MODIFIED> fail: hosp_code = %s, IO_MSG = %s, IO_FLAG = %s"
									, hospCode, pResult.getStrResult1() == null ? "null" : pResult.getStrResult1(), pResult.getStrResult2() == null ? "null" : pResult.getStrResult2()));
							response.setResult(false);
							return response;
						}
	...
	private Ocs2015 insertOcs2015(String hospCode, String userId, Double seq, Date actDate, Double pkocs2015, Double fkocs2005, String startTime, String endTime, Date endDate, OcsiModelProto.OCS6010U10frmARActinggrdOCS2015Info info){
		...
		ocs2015.setInputDoctor(info.getInputDocto());
		...
		ocs2015.setDv(CommonUtils.parseDouble(info.getDv()));			
		ocs2015.setChangeQty(CommonUtils.parseDouble(info.getChangeQty()));		
		ocs2015.setFio2(CommonUtils.parseDouble(info.getFio2()));			
	...
	private boolean handleGrdOcs2016CaseAddedOcs2015(String hospCode, String userId, Double pkocs2015, List<OcsiModelProto.OCS6010U10frmARActinggrdOCS2016Info> lstOcs2016, String ocs2015Suryang){
		for (OcsiModelProto.OCS6010U10frmARActinggrdOCS2016Info info : lstOcs2016) {
			if(DataRowState.ADDED.getValue().equals(info.getDataRowState())){
				if(!"0".equals(ocs2015Suryang) || !"C".equals(info.getBomYn())){
