...
import nta.med.data.dao.phr.StandardFitnessStepRepositoryCustom;
import nta.med.data.model.phr.StandardFitnessStepInfo;

public class StandardFitnessStepRepositoryImpl implements StandardFitnessStepRepositoryCustom {
	...
	public List<StandardFitnessStepInfo> getStandardFitnessStepInfoByprofileIdAndDurationType(Long profileId, Long durationType) {
		...
		sql.append("     SELECT                                                                                            								");
		sql.append("       A.ID,   A.DATETIME_RECORD DATETIME_RECORD,  SUM(A.STEPS_COUNT),                                                              ");
		sql.append("       A.NOTE,                                                                                         								");
		...
		//sql.append("       DATE_FORMAT(A.DATETIME_RECORD, '%Y-%m-%d') PERDAY                                               								");
		sql.append("     FROM PHR_STANDARD_FITNESS_STEPS A                                                                 							");
		...
		List<StandardFitnessStepInfo> fitnessInfos = new JpaResultMapper().list(query, StandardFitnessStepInfo.class);
		return fitnessInfos;
	}
}
